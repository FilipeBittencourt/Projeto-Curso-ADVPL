...
namespace Facile.Financeiro.BoletoNetCore
{
    internal sealed class BancoBrasil : IBanco
    {
        ...
        public void FormataCedente()
        {
            ...
            contaBancaria.FormatarDados("PAGÁVEL EM QUALQUER BANCO ATÉ O VENCIMENTO. APÓS, ATUALIZE O BOLETO NO SITE BB.COM.BR", "", 8, false);
        ...
        public void LerDetalheRetornoREC390SegmentoG(ref Pagamento pagamento, string registro)
        {
            try
            {
                pagamento.CodigoBarras = registro.Substring(199, 44);
                pagamento.CPFCNPJFavorecido = registro.Substring(36, 14);
                pagamento.NumeroDocumento = registro.Substring(50, 15);

                pagamento.CodigoUF = registro.Substring(1, 3);
                pagamento.CodigoReceita = registro.Substring(29, 6);
                pagamento.IdentificadorGuia = registro.Substring(4, 25);
                pagamento.PeriodoReferencia = registro.Substring(65, 6);

                //Valor do Título
                pagamento.ValorTitulo = Convert.ToDecimal(registro.Substring(71, 11)) / 100;
                pagamento.ValorAtualizacaoMonetaria = Convert.ToDecimal(registro.Substring(82, 11)) / 100;
                pagamento.ValorJuros = Convert.ToDecimal(registro.Substring(93, 11)) / 100;
                pagamento.ValorMulta = Convert.ToDecimal(registro.Substring(104, 11)) / 100;
                pagamento.ValorTotal = Convert.ToDecimal(registro.Substring(115, 11)) / 100;

                pagamento.DataVencimento = DataUtil.SQLToDate(registro.Substring(126, 8));
                pagamento.DataPagamento = DataUtil.SQLToDate(registro.Substring(134, 8));
                pagamento.DataAgendamento = DataUtil.SQLToDate(registro.Substring(332, 8));

                pagamento.AutorizacaoDebito = registro.Substring(243, 25);
                pagamento.AutenticacaoBancaria = registro.Substring(300, 17);
                pagamento.NumeroAgendamentoRemessa = registro.Substring(340, 5);
