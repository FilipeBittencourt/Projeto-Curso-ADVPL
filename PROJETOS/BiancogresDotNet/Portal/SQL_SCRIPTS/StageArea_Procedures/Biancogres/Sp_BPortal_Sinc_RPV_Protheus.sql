...
alter procedure Sp_BPortal_Sinc_RPV_Protheus
with encryption
as
begin
...
begin
	...
	select  
				...
				0
				from HADES.DADOSADV.dbo.BZINTEGRACAO_PREAE PRE_AE WITH (nolock)
				INNER JOIN HADES.DADOSADV.dbo.SC3010 SC3  WITH (nolock) ON 
					PRE_AE.CONTRATO			= SC3.C3_NUM 
					AND PRE_AE.ITEM 		= SC3.C3_ITEM 
					AND PRE_AE.COD_PRODUTO	= SC3.C3_PRODUTO
					AND SC3.D_E_L_E_T_ 		= ''''
				INNER JOIN HADES.DADOSADV.dbo.SA2010 SA2  WITH (nolock) ON A2_COD+A2_LOJA = C3_FORNECE+C3_LOJA AND SA2.D_E_L_E_T_ = ''''
				WHERE 1=1
				and not exists (
				select 1 from RPV t where ChaveUnica = RTRIM(PRE_AE.ID) collate Latin1_General_BIN
				)
	'
...
select  
			...
			INNER JOIN HADES.DADOSADV.dbo.SC3010 SC3  WITH (nolock) ON 
					...
					AND SC3.D_E_L_E_T_ 		= ''
			INNER JOIN HADES.DADOSADV.dbo.SA2010 SA2  WITH (nolock) ON A2_COD+A2_LOJA = C3_FORNECE+C3_LOJA AND SA2.D_E_L_E_T_ = ''
			INNER JOIN RPV f WITH (nolock) on f.ChaveUnica = RTRIM(PRE_AE.ID) collate Latin1_General_BIN
			WHERE 1=1
			and (
...
begin
	...
	if (not exists (select 1 from RPV where ChaveUnica = @ChaveUnica ))
	begin
