...
Local lRet		:= .T.
local lVerPerg := .T.
Local lEnd			:= .F. //@@ Customizado (o padrao nao estava criando essa variavel)
...
If nTipo <> 1
	...
	//@@@ Customização
	If allTrim(FunName()) $ "FINA090/VIXA104" .or.( IsBlind() .and. Type('oSetup') <> 'O' )

		/***********************
			PROJETO DE GUIAS GNRE
		/**********************/
		//oSetup := FWPrintSetup():New(16,"Printer")

	EndIf

	oDanfe:lServer := .t.// oSetup:GetProperty(PD_DESTINATION)==AMB_SERVER

	If IsBlind() .or. allTrim(FunName()) == "VIXA105"

		//Cria pasta caso nao exista
		MakeDir("\DANFES\")

		oDanfe:nDevice 	:= IMP_PDF
		oDanfe:cPathPDF	:= "\DANFES\"
		oDanfe:lServer 	:= .T.

		oDanfe:SetViewPDF(.F.)

	ElseIf  allTrim(FunName()) <> "FINA090" .AND. allTrim(FunName()) <> "VIXA104" .AND. allTrim(FunName()) <> "VIXA105" .and. Type('oSetup') == 'O'
	...
	Else
		...
		//@@@ Customização
		//Esse pergunte eh chamado customizado na funcao DANFEProc
		/*
		If lVerPerg
	...
	//@@@ Customização
	If IsBlind()
		DanfeProc(@oDanfe,@lEnd,cIdEnt,,,@lExistNfe, lIsLoja)
	Else
		RPTStatus( {|lEnd| DANFEProc(@oDanfe, @lEnd, cIDEnt, Nil, Nil, @lExistNFe, lIsLoja)}, "Imprimindo DANFE..." )
	EndIf
	//@@@ Fim
	...
	If lExistNFe
		...
		//@@@ Customização
		//PROJETO DE GUIAS GNRE
			If allTrim(FunName()) $ "FINA090/VIXA104" .or. isblind()
				oDanfe:Print()
			ElseIf alltrim(funname()) <> 'VIXA133' .and. alltrim(funname()) <> 'VIXA270'
				oDanfe:Preview() //Visualiza antes de imprimir
		//@@@ Fim
		EndIf

	Else
		If !lIsLoja .Or. !IsBlind()
	...
	/*
	//@@@ Inclusão
	//Inclusão Anderson Andrade
	//Data 12/11/2013
	*-------------------------------------------------------
	//IMPRESSAO DO BOLETO
	*-------------------------------------------------------
	If mv_par04 = 2 // notas de saida
		U_SRA0059(MV_PAR01,MV_PAR02,MV_PAR03)
	EndIf
	//@@@ Fim
	*/
	RestArea(aArea)
...
Local cChavSF3	:= ""
	...
	Local cTipo
	Local nPos
	Local _cPerg := PADR("NFSIGW",LEN(SX1->X1_GRUPO))
	//@@@ Fim
...
Default nTipo	:= 0
	...
	If IsBlind()
		...
		mv_par03 := SF2->F2_SERIE
		mv_par04 := 2 						//2 = saida
		mv_par05 := 1 						//1 = Nao imprime no verso
		MV_PAR06 := 2

	ElseIf alltrim(funname()) $ 'VIXENVSF,VIXNFENT,VIXA133'

		If Type('XTIPSF') = 'C' .and. xTipSF = 'E'

			mv_par01 := SF1->F1_DOC
			mv_par02 := SF1->F1_DOC
			mv_par03 := SF1->F1_SERIE
			mv_par04 := 1 						//1 = ENTRADA
			mv_par05 := 1 						//1 = Nao imprime no verso
			MV_PAR06 := 2
	...
	Else
		...
		/***********************
		  PROJETO DE GUIAS GNRE
		/**********************/
		If AllTrim(FunName()) == 'VIXA104' .OR. AllTrim(FunName()) == 'VIXA105' .OR. AllTrim(FunName()) == 'VIXA185' .OR. AllTrim(FunName()) == 'VIXA133'
			Pergunte(_cperg,.F.)
		ElseIf  !allTrim(FunName()) $ "FINA090"
			Pergunte(_cperg,!IsBlind())
		Else

			cTiposDoc 	:= AllTrim(SuperGetMv('MV_ESPECIE',,""))
			cTipo  	:= "SPED"
			nPos 		:= Rat( "=SPED", cTiposDoc)

			If nPos > 0
				cTiposDoc := substr(ALLTRIM(cTiposDoc),nPos-1,1)
			Else
				Alert("Não foi possível identificar a Serie da Nota Fiscal, a nota não será impressa.")
				Return
			EndIf

			MV_PAR03 := cTiposDoc+Space(2)
			MV_PAR01 := SE2->E2_NUM
			MV_PAR02 := SE2->E2_NUM
			MV_PAR04 := 2
			MV_PAR05 := 2
			...
			For nX := 1 To nLenNotas
				If !Empty(aXML[nX][2])
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
						...
						//@@@ Customização
						//Marcos Vieira-ERPI 16/05/2013
						//Posiciona arquivo para a personalizacao
						cChaveF2 := XFilial("SD2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]
						SD2->(DbSetOrder(3))
						SD2->(MsSeek(XFilial("SD2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]))
						SC5->(DbSetOrder(1))
						SC5->(MsSeek(XFilial("SC5")+SD2->D2_PEDIDO))

						SA1->(DbSetOrder(1))
						SA1->(DbSeek(XFilial("SA1")+aNotas[nX][06]+aNotas[nX][07]))
						//@@@ Fim
						...
						oNfeDPEC := XmlParser(EncodeUTF8( aXML[nX][4]),"_",@cAviso,@cErro)
...
aTransp := {"","0","","","","","","","","","","","","","",""}
	...
	//@@@ Customização
	//Marcos Vieira-ERPI 16/05/2013
	//Solicitado por Fabio em 04/11/11
	//Estava trazendo sempre o transportador 1 quando nao havia o mesmo - Anderson
	aTransp := {"",IIF(oTransp:_ModFrete:TEXT=="0","0","1"),"","","","","","","","","","","","","",""}
	//@@@ Fim
...
// @@ ALTERAÇÃO - MAURICIO 27/10/2017
If SC5->(FieldPos("C5_YPEDWEB")) > 0 .AND. !Empty(SC5->C5_YPEDWEB)

	oDanfe:Say(nLin,002,"Pedido Web: " + SC5->C5_YPEDWEB,oFont18N:oFont)

Else
	oDanfe:Say(nLin,002,"Pedido : " + SC5->C5_NUM ,oFont18N:oFont)
EndIf
...
oDanfe:EndPage()
//@@@ Customização
//Impressão de boletos - Kuhn
	If mv_par04 == 2 .and. !isblind() .AND. !FunName() $ 'VIXA104'

		aArea := GetArea()
		DbSelectArea('SE1')
		DbSetOrder(1)
	//PREFIXO, NUM, PARCELA, TIPO
		lAchou := SE1->(DbSeek(xFilial('SE1')+SF2->F2_SERIE+SF2->F2_DOC+PADR('A', AVSX3('E1_PARCELA', 3), ' ')+'NF'))
		If !lAchou
			lAchou := SE1->(DbSeek(xFilial('SE1')+SF2->F2_SERIE+SF2->F2_DOC+PADR(' ', AVSX3('E1_PARCELA', 3), ' ')+'NF'))
		EndIf

		lEnvEmail := AllTrim(SE1->E1_NUMBCO) == ''

		U_SRA0059(SF2->F2_DOC,SF2->F2_DOC,SF2->F2_SERIE,@oDanfe)

		lEnvBol := SE1->E1_SALDO != 0 .AND. !(SE1->E1_TIPO $ ('CH/CC/CD/RS')) .AND. SF2->F2_COND != '001' .AND. SA1->A1_YBOLIMP = '1'
		If lEnvBol
			If !lEnvEmail .AND. AllTrim(FunName()) != 'VIXA185'
				If MsgYesNo('Gostaria de enviar o boleto bancário(caso exista) e o DANFE por e-mail?')
					startjob("U_VIXA105",getenvserver(),.F.,{cEmpAnt, cFilAnt, SF2->F2_CLIENTE, SF2->F2_LOJA, SF2->F2_DOC, SF2->F2_SERIE})
				//U_VIXA105({cEmpAnt, cFilAnt, SF2->F2_CLIENTE, SF2->F2_LOJA, SF2->F2_DOC, SF2->F2_SERIE})
				EndIf
			//
			Else
				startjob("U_VIXA105",getenvserver(),.F.,{cEmpAnt, cFilAnt, SF2->F2_CLIENTE, SF2->F2_LOJA, SF2->F2_DOC, SF2->F2_SERIE})
