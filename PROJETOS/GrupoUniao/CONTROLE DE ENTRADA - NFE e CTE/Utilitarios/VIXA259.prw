...
User Function VIXA259(cUsrLog, cPswLog, lObrigat, cMtvLog)
	...
	Private oSayMtv
	Private oGetMtv, cGetMtv := Space(50)
	...
	//Criando a janela
	DEFINE MSDIALOG oDlgPvt TITLE " Ação do documento" FROM 000, 000  TO nJanAltu, nJanLarg COLORS 0, 16777215 PIXEL

	//Grupo de Login
	@ 003, 001     GROUP oGrpLog TO (nJanAltu/2)-1, (nJanLarg/2)-3         PROMPT "Dados: "     OF oDlgPvt COLOR 0, 16777215 PIXEL
	//Label e Get de Usuário
	@ 013, 006   SAY   oSayUsr PROMPT "Usuário:"        SIZE 100, 007 OF oDlgPvt                    PIXEL
	@ 020, 006   MSGET oGetUsr VAR    cGetUsr           SIZE (nJanLarg/2)-12, 007 OF oDlgPvt COLORS 0, 16777215 PIXEL

	//Label e Get da Senha
	@ 033, 006   SAY   oSayPsw PROMPT "Senha:"          SIZE 100, 007 OF oDlgPvt                    PIXEL
	@ 040, 006   MSGET oGetPsw VAR    cGetPsw           SIZE (nJanLarg/2)-12, 007 OF oDlgPvt COLORS 0, 16777215 PIXEL PASSWORD
	
	//Label e Get da Motivo
	@ 053, 006   SAY   oSayMtv PROMPT "Motivo:"          SIZE 100, 007 OF oDlgPvt             PIXEL
	If(lObrigat)
		@ 060, 006   GET oGetMtv VAR    cGetMtv            OF oDlgPvt MULTILINE SIZE (nJanLarg/2)-12, 020   COLORS 0, 16777215 PIXEL
	Else
		@ 060, 006   GET oGetMtv VAR    cGetMtv            OF oDlgPvt MULTILINE SIZE (nJanLarg/2)-12, 020   COLORS 0, 16777215  READONLY HSCROLL PIXEL
	EndIf
	...
	//Botões
	@ (nJanAltu/2)-18, 006 BUTTON oBtnConf PROMPT "Confirmar"             SIZE (nJanLarg/2)-12, 015 OF oDlgPvt ACTION (U_VIXA259V(lObrigat)) PIXEL
...
User Function VIXA259V(lObrigat)

	 
	Local cUsrAux := Alltrim(cGetUsr)
	Local cPswAux := Alltrim(cGetPsw)
	Local cMtvAux := Alltrim(cGetMtv)
	Local cCodAux := ""
	...
	If !Empty(cUsrAux) .And. PswSeek(cUsrAux)
		...
		If Empty(cMtvAux) .AND. lObrigat
			cGetErr := "Favor informar o motivo!"
			oGetErr:Refresh()		   
			Return  
		Else
			lRetorno := .T.
...
User Function VIX259GF()
	...
	Else
		...
		Aviso("ATENCAO", "Rotina deve ser usada em modo alteração!", {"Ok"})
