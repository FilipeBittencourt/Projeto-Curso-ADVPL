...
/*/{Protheus.doc} BIAF155
@author Tiago Rossini Coradini
@since 19/09/2019
@version 1.0
@description Função para gerção via Job dos relatorios de Perda - BIAV002
@obs Ticket: 23030
@type function
/*/

User Function BIAF155()
Local aEmp := {}
Local cFil := "01"
Local nCount := 0
Private aBkpPar := {}
...
Local dDtFin := dDataBase
	...
	dDtIni := cToD("01/10/" + cValToChar(Year(YearSub(dDataBase, 1))))
	dDtFin := LastDate(MonthSum(dDtIni, Month(dDataBase) -1))
...
User function wfSend02()
	Local df_Dest := ""
	Local df_Assu := "Relatórios de Liquidez - BIAV002 "
	Local df_Erro := df_Assu + " não enviado. Favor verificar!!!" 

	df_Dest:=U_EmailWF('BIAF155', cEmpAnt)
	
	U_BIAEnvMail(, df_Dest, df_Assu, "Relatório de Perda atualizado.", df_Erro)	
return
