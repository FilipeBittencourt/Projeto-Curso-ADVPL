...
User Function WORK_VEND()
	...
	For x := 1 to Len(xv_Emps)
		...
		ConOut("HORA: "+TIME()+" - GERANDO WORKFLOW DE VENDEDORES " + xv_Emps[x,1])

		Processa({||WORKVEND()})

		ConOut("HORA: "+TIME()+" - FIM DO WORKFLOW DE VENDEDORES " + xv_Emps[x,1])
...
Static Function WORKVEND()
	...
	Private CCC_TEXTO	:= ""
	Private lDebugWF := .F.
    Private _cDirDest := "C:\TEMP\"

	//Nome temporario para as tabelas
	nNomeSC5	:= "TMPRAC_WORK_VEND_SC5"+cEmpAnt+__cUserID+strzero(seconds()*3500,10)//strzero(seconds()*3500,10)
	nNomeSC6	:= "TMPRAC_WORK_VEND_SC6"+cEmpAnt+__cUserID+strzero(seconds()*3500,10)//Alltrim(Str(randomize(1,34000)))
	nNomeSC9	:= "TMPRAC_WORK_VEND_SC9"+cEmpAnt+__cUserID+strzero(seconds()*3500,10)//strzero(seconds()*3500,10)
	nNomeSE1	:= ""
	aRet := {}
	...
	If lDebugWF
		nHandle := FCreate(_cDirDest+"consultaSQL1.txt")
		FWrite(nHandle, cSql)
		FClose(nHandle)     
    EndIf
	...
	If lDebugWF
		nHandle := FCreate(_cDirDest+"consultaSQL2.txt")
		FWrite(nHandle, cSql)
		FClose(nHandle)     
    EndIf
	...
	//Ticket 32029 - Alterar para que leitura de Data de Necessidade seja sempre pela fabricante (Bianco, Incesa...).
	...
	If cEmpAnt $ "01_05_14"
		...
		If cEmpAnt == "14"
			...
			cSql += "SELECT '14' AS 'EMPRESA' , C6_FILIAL,	C6_ITEM,	C6_PRODUTO,	C6_YEMP, C6_DESCRI,	C6_UM,	C6_QTDVEN,	C6_PRCVEN,	C6_VALOR,	C6_QTDLIB,	C6_TES,	C6_CF,	C6_SEGUM,	C6_UNSVEN,	C6_LOCAL,	C6_QTDEMP,	C6_QTDENT,	C6_CLI,	" + Enter
			cSql += "		C6_YPERC,	C6_YDESC,	C6_DESCONT,	C6_VALDESC,	(CASE WHEN C6_YDTNECE <> '' THEN C6_YDTNECE ELSE C6_ENTREG END) AS C6_ENTREG, C6_YDTNECE, C6_LA,	C6_LOJA,	C6_NOTA,	C6_SERIE,	C6_DATFAT,	C6_NUM,	C6_COMIS1,	C6_COMIS2,	C6_COMIS3,	" + Enter
			cSql += "		C6_COMIS4,	C6_COMIS5,	C6_YPALLET,	C6_PEDCLI,	C6_PRUNIT,	C6_BLOQUEI,	C6_GEROUPV,	C6_RESERVA,	C6_OP,	C6_OK,	C6_NFORI,	C6_SERIORI,	C6_ITEMORI,	C6_IPIDEV,	C6_IDENTB6,	C6_BLQ,	C6_PICMRET,	C6_CODISS,	C6_GRADE,	C6_ITEMGRD,	C6_LOTECTL,	" + Enter
			cSql += "		C6_NUMLOTE,	C6_DTVALID,	C6_NUMORC,	C6_CHASSI,	C6_OPC,	C6_LOCALIZ,	C6_NUMSERI,	C6_NUMOP,	C6_ITEMOP,	C6_CLASFIS,	C6_YPESOL,	C6_QTDRESE,	C6_NUMOS,	C6_NUMOSFA,	C6_CODFAB,	C6_LOJAFA,	C6_TPOP,	C6_REVISAO,	C6_YEMISSA,	C6_USERLGI,	" + Enter
			cSql += "		C6_USERLGA,	C6_YIMPNF,	C6_QTDLIB2,	C6_QTDENT2,	C6_SERVIC,	C6_ENDPAD,	C6_CONTRT,	C6_TPCONTR,	C6_ITCONTR,	C6_PROJPMS,	C6_TASKPMS,	C6_LICITA,	C6_QTDEMP2,	C6_CONTRAT,	C6_ITEMCON,	C6_TPESTR,	C6_EDTPMS,	C6_TRT,	C6_PROJET,	C6_ITPROJ,	" + Enter
	...
	If lDebugWF
		nHandle := FCreate(_cDirDest+"consultaSQL3.txt")
		FWrite(nHandle, cSql)
		FClose(nHandle)     
    EndIf
	...
	If lDebugWF
		nHandle := FCreate(_cDirDest+"consultaSQL4.txt")
		FWrite(nHandle, cSql)
		FClose(nHandle)     
    EndIf
	...
	DO WHILE ! _TRAB->(EOF()) 
		...
		CEMAIL	:= ALLTRIM(_TRAB->A3_EMAIL)
		CCODIGO := ALLTRIM(_TRAB->A3_COD)
		CNOME 	:= ALLTRIM(_TRAB->A3_NOME)

		ConOut("HORA: "+TIME()+" - VENDEDOR -> " + CCODIGO + "-" + CNOME)
		...
		//Ticket 31799 - considerar apenas a data de liberaÁ„o como data do empenho
		cSql += "				DIAS_NECE = CASE WHEN C6_YDTNECE <> '' THEN DATEDIFF(D,C6_YDTNECE, GETDATE()) 	ELSE DATEDIFF(D,C9_DATALIB, GETDATE()) END, " + Enter
		cSql += "				DIAS = DATEDIFF(D,C9_DATALIB, GETDATE()) " + Enter
		

		cSql += " FROM	" + Enter	
		//(Thiago Dantas - 27/03/15)
		...
		cSql += " AND ((C6_YDTNECE <> '' AND DIAS_NECE >= -30) OR C6_YDTNECE = '') " + Enter
		...
		If lDebugWF
			nHandle := FCreate(_cDirDest+"consultaSQL5.txt")
			FWrite(nHandle, cSql)
			FClose(nHandle)     
		EndIf
		...
		IF ! __TRAB_AUX->(EOF())
			IF ALLTRIM(CEMAIL) <> ""
				...
				C_HTML += ' <head> '
				C_HTML += ' <title>Relacao Peso Disponivel</title> '
				C_HTML += ' <style type="text/css"> '
				C_HTML += ' <!-- '
				C_HTML += ' .style12 {font: 9px Arial, Helvetica, sans-serif } '
				C_HTML += ' .style18 {font: 10px Arial, Helvetica, sans-serif} '
				C_HTML += ' .style21 {font-weight: bold; color: #FFFFFF; font: 9px Arial, Helvetica, sans-serif } '
				C_HTML += ' .style22 { '
				C_HTML += '	font-weight: bold; font: 11px Arial, Helvetica, sans-serif'
				C_HTML += ' } '
				C_HTML += ' .style35 {font: 10px Arial, Helvetica, sans-serif} '
				C_HTML += ' .style36 {font: 9px Arial, Helvetica, sans-serif } '
				C_HTML += ' .style39 {font: 12px Arial, Helvetica, sans-serif } '
				C_HTML += ' --> '
				C_HTML += ' </style> '
				C_HTML += ' </head> '
				C_HTML += ' <body> '
				...
				C_HTML += '  <tr> '
				IF CEMPANT = "01"
				C_HTML += '    <th width="236" scope="col"> <font face="Arial"> BIANCOGRES CERAMICA SA </font> </th> '
				ELSEIF CEMPANT = "05"
					C_HTML += '    <th width="236" scope="col"> <font face="Arial"> INCESA CERAMICA LTDA </font> </th> '
				ELSEIF CEMPANT = "13"
					C_HTML += '    <th width="236" scope="col"> <font face="Arial"> MUNDI LOGISTICA LTDA </font> </th> '
				END IF
				C_HTML += '    <th width="520" rowspan="3" scope="col"> <font face="Arial"> RELACAO PESO DISPONIVEL POR VENDEDOR </font> </th> '
				C_HTML += '    <td class="style12"><div align="right"> DATA EMISSAO: '+ dtoC(DDATABASE) +' </div></td> '
				C_HTML += '  </tr> '
				C_HTML += '  <tr> '
				C_HTML += '    <td>&nbsp;</td> '
				C_HTML += '    <td class="style12"><div align="right"> HORA DA EMISSAO: '+SUBS(TIME(),1,8)+' </div></td> '
				C_HTML += '  </tr> '
				C_HTML += '  <tr> '
				C_HTML += '    <td>&nbsp;</td> '
				C_HTML += '    <td>&nbsp;</td> '
				...
				C_HTML += '    <th colspan="11" scope="col"><div align="left" class="style35">Vendedor: '+ AllTrim(__TRAB_AUX->A3_COD)+' - '+AllTrim( __TRAB_AUX->A3_NOME) +' - '+ AllTrim(__TRAB_AUX->A3_NREDUZ) +' </div></th> '
				C_HTML += '  </tr> '

				// CABECALHO
				C_HTML += '  <tr bgcolor="#0066CC"> '
				C_HTML += '    <th width="20"	scope="col"><span class="style21"> Lc </span></th> '
				C_HTML += '    <th width="385"	scope="col"><span class="style21"> Cliente </span></th> '
				C_HTML += '    <th width="100" 	scope="col"><span class="style21"> Cidade </span></th> '
				C_HTML += '    <th width="25" 	scope="col"><span class="style21"> UF </span></th> '
				C_HTML += '    <th width="55" 	scope="col"><div align="center"><span class="style21"> Pedido </span></div></th> '
				C_HTML += '    <th width="55" 	scope="col"><div align="center"><span class="style21"> OC </span></div></th> '
				C_HTML += '    <th width="55" 	scope="col"><div align="center"><span class="style21"> Dt. Prev. Entrega </span></div></th> '
				...
				C_HTML += '    <th width="48" 	scope="col"><span class="style21"> Dias de Empenho </span></th> '
				C_HTML += '  </tr> '
				...
				DO WHILE ! __TRAB_AUX->(EOF())
					...
					While (cCliAux == __TRAB_AUX->A1_COD ) .And. !__TRAB_AUX->(EOF())
						...
						C_HTML += '  <tr> '
						C_HTML += '	   <td class="style12"> '+Alltrim(__TRAB_AUX->EMPRESA)+' </td> '
						C_HTML += '    <td class="style12"> '+Alltrim(__TRAB_AUX->A1_COD)+' - '+Alltrim(__TRAB_AUX->A1_NOME)+' </td> '
						C_HTML += '    <td class="style12"> '+__TRAB_AUX->A1_MUN+' </td> '
						C_HTML += '    <td class="style12"> <div align="center"> '+Alltrim(__TRAB_AUX->A1_EST)+' </div></td> '
						C_HTML += '    <td class="style12"> <div align="center"> '+Alltrim(__TRAB_AUX->C5_NUM)+' </div></td> '
						C_HTML += '    <td class="style12"> <div align="center"> '+Alltrim(__TRAB_AUX->C5_YPC)+' </div></td> '
						C_HTML += '    <td class="style12"> <div align="center"> '+dtoc(stod(IIF(__TRAB_AUX->DTDISPOP > __TRAB_AUX->C6_ENTREG .And. __TRAB_AUX->C6_YDTNECE = '' , __TRAB_AUX->DTDISPOP, __TRAB_AUX->C6_ENTREG)))+' </div></td> '

						C_HTML += '    <td class="style12"> '+Alltrim(__TRAB_AUX->C6_DESCRI)+' </td> '
						C_HTML += '    <td class="style12"> <div align="right"> '+TRANSFORM(__TRAB_AUX->C9_QTDLIB		,"@E 999,999,999")+' </div></td> '
						C_HTML += '    <td class="style12"> <div align="right"> '+TRANSFORM(__TRAB_AUX->Z9_PESOBR		,"@E 999,999,999")+' </div></td> '
						C_HTML += '    <td class="style12"> <div align="center">'+TRANSFORM(__TRAB_AUX->DIAS			,"@E 999,999,999")+' </div></td> '
						C_HTML += '  </tr> '
				...
				C_HTML += '  <tr bgcolor="#FFFFFF"> '
				C_HTML += '    <th colspan="7" scope="col"><div align="left" class="style35">Total Vendedor: '+AllTrim(SS_VEND)+' </div></th> '
				...
				C_HTML += '    <td class="style35"> <div align="right"> '+TRANSFORM(NTOTAL_GERAL ,"@E 999,999,999")+' </div></td> '
			...
			ELSE
				CCC_TEXTO += 'EMAIL N√O CADASTRADO NO VENDEDOR: ' + CCODIGO +' - '+ CNOME + Enter
			END IF
		END IF
		_TRAB->(DBSKIP())
		C_HTML := ""	
...
Static Function fImpTotCli()
	...
	C_HTML += '  <tr bgcolor="#FFFFFF"> '
	C_HTML += '    <th colspan="7" scope="col"><div align="left" class="style35">Total Cliente: '+AllTrim(cCliAux)+' - '+AllTrim(cNomeCli)+' </div></th> '
	C_HTML += '    <td class="style35"> <div align="right"> </div></td> '
	C_HTML += '    <td class="style35"> <div align="right"> '+TRANSFORM(NQtdCli ,"@E 999,999,999")+' </div></td> '
	C_HTML += '    <td class="style35"> <div align="right"> '+TRANSFORM(NTotalCli ,"@E 999,999,999")+' </div></td> '
	C_HTML += '    <td class="style35"> <div align="center"> '+TRANSFORM(NDiasCli / NQuantCli,"@E 999,999,999")+' </div></td> '
	C_HTML += '  </tr> '
...
STATIC FUNCTION COMIS_EMAIL()
	...
	IF CEMPANT == "01"
		cRecebe		:= CEMAIL
		cRecebeCC	:= "expedicao@biancogres.com.br"
		cAssunto	:= "BIANCOGRES - RELACAO PESO DISPONIVEL POR VENDEDOR "
	ELSEIF CEMPANT == "05"
		cRecebe   	:= CEMAIL
		cRecebeCC	:= ""
		cAssunto	:= "INCESA - RELACAO PESO DISPONIVEL POR VENDEDOR "
	ELSEIF CEMPANT == "13"
		cRecebe   	:= CEMAIL
		cRecebeCC	:= "paloma.santos@biancogres.com.br"
		cAssunto	:= "MUNDI - RELACAO PESO DISPONIVEL POR VENDEDOR "
	ELSEIF CEMPANT == "14"
		cRecebe   	:= CEMAIL
		cRecebeCC	:= "paloma.santos@biancogres.com.br"
		cAssunto	:= "VINILICO - RELACAO PESO DISPONIVEL POR VENDEDOR "	
	END IF
	...
	IF !lOK 
		EMA_N_CADAS("EMAIL INV¡LIDO: " + __TRAB_AUX->A3_COD+' - '+ __TRAB_AUX->A3_NOME + ' - ' + CEMAIL)
	ENDIF
...
STATIC FUNCTION EMA_N_CADAS(CCMENSAGEM)
	...
	IF CEMPANT == "01"
		cRecebe     := "expedicao@biancogres.com.br" 	// CEMAIL
		cRecebeCC	:= ""
	ELSE
		cRecebe     := "ana.silva@biancogres.com.br" 	// CEMAIL
		cRecebeCC	:= ""
	END IF
	cAssunto	:= 'WORKFLOW - EMAIL N√O CADASTRADO NO VENDEDOR: ' //+ CCODIGO +' - '+ CNOME
	...
	lOk := U_BIAEnvMail(,cRecebe,cAssunto,CCMENSAGEM,,,.F.,cRecebeCC)
