...
Static Function WORK_TRAN()
	...
	cSql += "	TMP.C6_YDTNECE AS C6_YDTNECE, TMP.C6_LA,	TMP.C6_LOJA, TMP.C6_NOTA, TMP.C6_SERIE,	TMP.C6_DATFAT, TMP.C6_NUM, TMP.C6_COMIS1, TMP.C6_COMIS2, TMP.C6_COMIS3,	TMP.C6_COMIS4, TMP.C6_COMIS5, TMP.C6_YPALLET, " + Enter
	...
	DO WHILE ! S_TRAB->(EOF())
		...
		cSql += " AND ((C6_YDTNECE <> '' AND DIAS_NECE >= -7) OR C6_YDTNECE = '') " + Enter
		...
		IF ! _TRAB_AUX->(EOF())
			...
			IF ALLTRIM(CEMAIL) <> ""
				...
				C_HTML += '    <td width="189" class="style12"><div align="right"> DATA EMISS√O: '+ dtoC(DDATABASE) +' </div></td> '
				C_HTML += '  </tr> '
				C_HTML += '  <tr> '
				...
				COMIS_EMAIL()
			ELSE
				CCC_TEXTO += 'EMAIL N√O CADASTRADO NA TRANSPORTADORA: ' + CCODIGO +' - '+ CNOME + Enter
			END IF
		END IF
		DBSELECTAREA("S_TRAB")
		S_TRAB->(DBSKIP())
...
STATIC FUNCTION COMIS_EMAIL()
	...
	ELSEIF CEMPANT = "14"
		cRecebe		:= CEMAIL
		cRecebeCC	:= "paloma.santos@biancogres.com.br" 
		cAssunto	:= 'VINILICO - Relacao Peso Disponivel por Transportadora '		
	ENDIF
	...
	IF !lOK
		EMA_N_CADAS("EMAIL INV¡LIDO: " + _TRAB_AUX->A4_COD +' - '+ _TRAB_AUX->A4_NOME + ' - ' + CEMAIL )
	ENDIF
...
STATIC FUNCTION EMA_N_CADAS(CCMENSAGEM)  
	...
	cAssunto	:= 'EMAIL N√O ENVIADOS PARA TRANSPORTADORA: ' //+ CCODIGO +' - '+ CNOME
