...
/*/
//--------------------------------------------------------------------
User Function UPDZG8( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDZG8" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDZG8" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'R'																	, .F. }, ; //X3_CONTEXT
	{ '€'																	, .F. }, ; //X3_OBRIGAT
	{ 'ExistCpo("SX5","12"+M->ZG8_UF) .And. ExistChav("ZG8",M->ZG8_UF,1,"ZG8_UF")', .F. }, ; //X3_VLDUSER
	{ ''																	, .F. }, ; //X3_CBOX
	{ ''																	, .F. }, ; //X3_CBOXSPA
	{ ''																	, .F. }, ; //X3_CBOXENG
	{ ''																	, .F. }, ; //X3_PICTVAR
	{ 'IIF(!INCLUI,.F.,.T.)'												, .F. }, ; //X3_WHEN
	{ ''																	, .F. }, ; //X3_INIBRW
	{ ''																	, .F. }, ; //X3_GRPSXG
	{ ''																	, .F. }, ; //X3_FOLDER
	{ ''																	, .F. }, ; //X3_CONDSQL
...
aAdd( aSX3, { ;
	...
	{ 5																		, .F. }, ; //X3_TAMANHO
	{ 2																		, .F. }, ; //X3_DECIMAL
	{ '% Tarifa Seg'														, .F. }, ; //X3_TITULO
	{ '% Tarifa Seg'														, .F. }, ; //X3_TITSPA
	{ '% Tarifa Seg'														, .F. }, ; //X3_TITENG
	{ '% Tarifa Seguro'														, .F. }, ; //X3_DESCRIC
	{ '% Tarifa Seguro'														, .F. }, ; //X3_DESCSPA
	{ '% Tarifa Seguro'														, .F. }, ; //X3_DESCENG
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDZG8" ) ) ) ;
