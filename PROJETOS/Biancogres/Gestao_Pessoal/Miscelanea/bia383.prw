...
@since 13/09/17
@version 1.0
@description Tela para cadastro das metricas orçamentárias de Encargos Trabalhistas 
@type function
/*/
...
User Function B383FOK()
	...
	Do Case
		...
		If Empty(M->ZB2_ENCARG)
			MsgInfo("Necessário informar o Encargo!!!")
			Return .F.
		EndIf
		...
		If Empty(M->ZB2_CATGFU)
			MsgInfo("Necessário informar a Categoria do funcionário que aplica o Encargo!!!")
			Return .F.
		EndIf
		If !ExistCPO("ZB4")
			MsgInfo("Registro não cadastrado!!!")
			Return .F.
		EndIf
		...
		If M->ZB2_PRCBAS == 0 .and. !cEmpAnt $ "07/13"
			MsgInfo("Necessário informar o % sobre o qual será aplicado o Encargo!!!")
			Return .F.
		EndIf
		...
		If _nI <> _nAt .and. !GDdeleted(_nI)
			...
			If !Empty(_gbENCARG) .and. _gbENCARG == GdFieldGet("ZB2_ENCARG",_nI)
				...
				If !Empty(_gbCATGFU) .and. _gbCATGFU == GdFieldGet("ZB2_CATGFU",_nI)
					...
					MsgInfo("Não poderá haver a mesma Categoria Func informada para o mesmo Encargo mais de uma vez na lista. Na linha: " + Alltrim(Str(_nI)) + " já existe a Categoria Func informada!!!")
					Return .F.
...
User Function B383TOK()
	...
	Local _msMensErr := "Os seguintes encargos não atingiram a quantidade de categorias cadastrada: " + _msEnter + _msEnter
...
User Function B383RPCSET()
	...
	fgContin := MsgYESNO("Você está prestes a executar o reprocessamento dos Encargos Trabalhistas. ESTÁ CERTO(A) DE QUE TODOS OS REGISTROS FORAM DEVIDAMENTE GRAVADOS??? Deseja continuar???")
	If !fgContin
	...
	Else
		...
		fgContin := MsgYESNO("Você CONFIRMOU o reprocessamento. Primeiramente o sistema irá excluir o desdobramento. Deseja continuar???")
		If !fgContin
	...
	MsgINFO("O processamento foi concluído. Favor verificar se os dados foram gravados corretamente...")

	MsgALERT("Este processamento deverá ser executado individualmente para cada uma das empresas contempladas pelo Orçamento RH")

	MsgINFO("Não se esqueça de processar para todas as empresa...")

	MsgALERT("Ah! Mais uma coisa... necessário processar o desdobramento em todas as empresas novamente...")
...
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
¦¦+-----------------------------------------------------------------------+¦¦
¦¦¦Funçao    ¦ B383IEXC ¦ Autor ¦ Marcos Alberto S      ¦ Data ¦ 21/06/17 ¦¦¦
¦¦+----------+------------------------------------------------------------¦¦¦
...
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
¦¦+-----------------------------------------------------------------------+¦¦
¦¦¦Funçao    ¦ B383RPLC ¦ Autor ¦ Marcos Alberto S      ¦ Data ¦ 09/09/19 ¦¦¦
¦¦+----------+------------------------------------------------------------¦¦¦
