...
@since 09/10/17
@version 1.0
@description Tela para Consulta do Orçamento RH em ORCA FINAL  
@type function
/*/

User Function BIA399()
	...
	Define MsDialog _oDlg Title "Consulta Orçamento RH em Despesas / Custo Fixo" From _aSize[7],0 To _aSize[6],_aSize[5] Of oMainWnd Pixel

	@ 050,010 SAY "Versão:" SIZE 55, 11 OF _oDlg PIXEL FONT oFont
	@ 048,050 MSGET _oGVersao VAR _cVersao Picture "@!" F3 "ZB5" SIZE 50, 11 OF _oDlg PIXEL VALID fBIA399A()

	@ 050,110 SAY "Revisão:" SIZE 55, 11 OF _oDlg PIXEL FONT oFont
	@ 048,150 MSGET _oGRevisa VAR _cRevisa  SIZE 50, 11 OF _oDlg PIXEL VALID fBIA399B()

	@ 050,210 SAY "AnoRef:" SIZE 55, 11 OF _oDlg PIXEL FONT oFont
	@ 048,250 MSGET _oGAnoRef VAR _cAnoRef  SIZE 50, 11 OF _oDlg PIXEL VALID fBIA399C()
...
Static Function fBIA399A()
	...
	If !Empty(_cVersao) .and. !Empty(_cRevisa) .and. !Empty(_cAnoRef)
		...
		Processa({ || cMsg := fBIA399C() }, "Aguarde...", "Carregando dados...",.F.)
	EndIf

Return .T.

Static Function fBIA399B()
...
Static Function fBIA399C()
	...
	M0007 += "          INNER JOIN LIBCLVL ZB9 ON ZB9.ZB9_CLVL = XZBZ.ZBZ_CLVLDB "
	...
	M0007 += "            AND XZBZ.ZBZ_ORIPRC = 'RH' "
	M0007 += "            AND XZBZ.D_E_L_E_T_ = ' ') NREGS "
	M0007 += "   FROM " + RetSqlName("ZBZ") + " ZBZ "
	M0007 += "  INNER JOIN LIBCLVL ZB9 ON ZB9.ZB9_CLVL = ZBZ.ZBZ_CLVLDB "
	...
	M0007 += "        (SELECT COUNT(*) "
	M0007 += "           FROM " + RetSqlName("ZBZ") + " XZBZ "
	M0007 += "          INNER JOIN LIBCLVL ZB9 ON ZB9.ZB9_CLVL = XZBZ.ZBZ_CLVLCR "
	M0007 += "          WHERE XZBZ.ZBZ_FILIAL = '" + xFilial("ZBZ") + "' "
	M0007 += "            AND XZBZ.ZBZ_VERSAO = '" + _cVersao + "' "
	M0007 += "            AND XZBZ.ZBZ_REVISA = '" + _cRevisa + "' "
	M0007 += "            AND XZBZ.ZBZ_ANOREF = '" + _cAnoRef + "' "
	...
	M0007 += "            AND XZBZ.D_E_L_E_T_ = ' ') NREGS "
	M0007 += "   FROM " + RetSqlName("ZBZ") + " ZBZ "
	M0007 += "  INNER JOIN LIBCLVL ZB9 ON ZB9.ZB9_CLVL = ZBZ.ZBZ_CLVLCR "
	M0007 += "  WHERE ZBZ.ZBZ_FILIAL = '" + xFilial("ZBZ") + "' "
	M0007 += "    AND ZBZ.ZBZ_VERSAO = '" + _cVersao + "' "
	M0007 += "    AND ZBZ.ZBZ_REVISA = '" + _cRevisa + "' "
	M0007 += "    AND ZBZ.ZBZ_ANOREF = '" + _cAnoRef + "' "
