...
If Pergunte("NFSIGW",.T.)
	...
	If (MV_PAR04==2)//nf saida
		lRetMDF := U_BIAVTMDF(MV_PAR01, MV_PAR02, MV_PAR03)
		If (lRetMDF)
			RestArea(aArea)
			lExistNfe := .F.
			Return(.T.)
				...
				If !Empty(aXML[nX][2]) .And. Empty((cAliasSFT)->FT_DTCANC)
					...
					If ( !Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5,7" ) .And. !cCodRetNFE $ RetCodDene()
						...
						ElseIf aNotas[nX][02]=="S" .And. MV_PAR04==2 .And. (oNfe:_NFE:_INFNFE:_IDE:_TPNF:TEXT=="1")
							...
							// BIANCOGRES -> CORRIGE O CAMPO F2_FIMP PARA AS NF'S AUTORIZADAS EM CONTINGENCIA
							If MsSeek(xFilial("SF2")+aNotas[nX][05]+aNotas[nX][04]) .And. Alltrim(aXML[nX][8]) == "2" .And. SF2->F2_FIMP == "T"
								RecLock("SF2")
								SF2->F2_FIMP := "S"
							EndIf
...
//TODO
If (cEmpAnt == '07')
	oDanfe:Say(nLinCalc-12,005, aTransp[13],oFont10N:oFont)
EndIf
...
ElseIf cEmpAnt == '01' .And. Alltrim(oNFe:_NFE:_INFNFE:_IDE:_SERIE:Text) == "4" //pegasus - bianco
	cLogo := ""	
	/*If (aTransp[13] == 'BIANCOGRES')
		cLogo := "\SYSTEM\logonfe01.bmp"
	ElseIf (aTransp[13] == 'INCESA')
		cLogo := "\SYSTEM\logonfe05.bmp"
	EndIf
	*/	
EndIf
...
If nfolha==1
	...
	ElseIf cEmpAnt <> "07"
		...
		If (cEmpAnt == '01' .And. Alltrim(oNFe:_NFE:_INFNFE:_IDE:_SERIE:Text) == "4" )//ajuste até nova marca ter logo
			oDanfe:Say(060, 005, "Pegasus", oFont18N:oFont)
		Else
