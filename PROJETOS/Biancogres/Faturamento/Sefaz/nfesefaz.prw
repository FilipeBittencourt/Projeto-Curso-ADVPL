...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						// BIANCOGRES - INICIO - BLQ001 - Comentado o bloco abaixo de mensagens padrao
						/*
						...
						*/
						...
						// BIANCOGRES - INICIO - BLQ002 - Comentado o bloco abaixo de mensagens padrao
						/*
			    ...
			    If lQuery
			    	...
			    	// BIANCOGRES - INICIO - BLQ010 - Bloco abaixo comentado, não é necessário fechar a area de trabalho do SC5
			    	/*		    		    
			    	dbSelectArea("SC5")
			    	dbCloseArea()
			    	// BIANCOGRES - FIM - BLQ010
			    	*/
			    EndIf
...
//BIANCOGRES ==> copiado todo o bloco do PE abaixo do fonte em uso antes da 4.0 , bloco original comentado acima
IF ExistBlock("PE01NFESEFAZ")                   

	aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aEspVol,aNfVincRur} // BIANCOGRES - BLQ003 - Adicionado array de especie e volume (aEspVol) ao ponto de entrada
	...
	If ( Len(aParam) >= 5 )
		...
		aVeiculo	:= aParam[11]
		aReboque	:= aParam[12]
		aEspVol	:= aParam[13] // BIANCOGRES - BLQ004 - Adicionado tratamento do array de especie e volume (aEspVol)
		aNfVincRur := aParam[14] // BIANCOGRES - BLQ005 - Adicionado tratamento do array de vinculo rural (aNfVincRur)
	EndIf
...
If !Empty(aNota)
	...
	cString += NfeInfAd(cMensCli,cMensFis,aPedido,aExp,cAnfavea,aMotivoCont,aNota,aNfVinc,aProd,aDI,aNfVincRur,aRetido,cNfRefcup,cSerRefcup,cTipo,nIPIConsig,nSTConsig,lBrinde,cVerAmb,Iif(aNota[5] == "D",aRefECF,{}),nVicmsDeson,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,nvBCUFDest,aICMUFDest,nValIpiBene,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aObsCont,cValTotOpe)
...
If cVeramb >= "3.10"
	...
	cString += NfeTag('<dhSaiEnt>',Iif(lDSaiEnt,"",ConvType(aNota[07])+"T"+Iif(Len(aNota[06]) > 5,ConvType(aNota[06]),ConvType(aNota[06])+":00"))) // BIANCOGRES - BLQ006 - Adiciona posicao 07 ao array para tratar data de saida/entrada - Comentada linha acima
Else	
...
If (AllTrim(SA1->A1_GRPVEN) == '000026') //Construdecor SA
	cString += '<vUnCom>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType( ROUND(aProd[10]/aProd[09], 2), 21, 8))+'</vUnCom>'
Else
...
If (AllTrim(SA1->A1_GRPVEN) == '000026') //Construdecor SA
	cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd), ConvType( ROUND(aProd[10]/aProd[12], 2) , 21, 8))+'</vUnTrib>'		
Else
...
If (AllTrim(SA1->A1_GRPVEN) == '000026') //Construdecor SA
	cVunCom := ConvType( ROUND(aProd[10]/aProd[09], 2), 21, 8)
	cVunTrib:= ConvType( ROUND(aProd[10]/aProd[12], 2), 21, 8)
Else
...
For nX := 1 To Len(aVol)		
	cString += '<vol>'
		...
		cString += NfeTag('<marca>',ConvType(aVol[nX][05])) // BIANCOGRES - BLQ008 - Adicionada posicao [05] no vetor aEspVol com informação da Marca
...
//Customizado: Ordenado por data num parcela - Jussara 19/07/2019
If Len(aDupl) > 1
	aDupl := aSort(aDupl,,,{|x,y| DToS(x[2]) + x[1] < DtoS(y[2]) + y[1]})
EndIf	
