...
#DEFINE MAXVALORC  020 // BIANCOGRES - Comentado acima



/*/{Protheus.doc} fautdanfe
@description Copia do programa original DANFE para funcoes de automacao do faturamento
@author Fernando Rocha
@since 29/01/2019
@version 1.0
@type function
/*/
User Function fautdanfe(cIdEnt,cVal1,cVal2,oDanfe,oSetup,cFilePrint, cNota, cSerie)
	...
	oDanfe:cPathPDF	:= "c:\movimento_bancario_033\"
	oDanfe:lViewPDF	:= .F.
	oDanfe:lServer	:= .F.
	oDanfe:setCopies(1)
	...
	*/
	...
	//RptStatus({|lEnd| DanfeProc(@oDanfe,@lEnd,cIdEnt,,,@lExistNfe,cCarga, cCodEmp)},"Imprimindo Danfe...")
	DanfeProc(@oDanfe,.F.,cIdEnt,,,@lExistNfe,cNota, cSerie)
...
Static Function DanfeProc(oDanfe,lEnd,cIdEnt,cVal1,cVal2,lExistNfe,cNota, cSerie)
	...
	BeginSql Alias cAliasSF3
		...
		SELECT * FROM
		(
			SELECT F2_YEMP, F3_FILIAL,F3_ENTRADA,F3_NFELETR,F3_CFO,F3_FORMUL,F3_NFISCAL,F3_SERIE,F3_CLIEFOR,F3_LOJA,F3_ESPECIE,F3_DTCANC
			FROM %Table:SF3% SF3
			JOIN %Table:SF2% SF2 ON F2_FILIAL = F3_FILIAL AND F2_DOC = F3_NFISCAL AND F2_SERIE = F3_SERIE AND F2_CLIENTE = F3_CLIEFOR AND F2_LOJA = F3_LOJA AND SF2.D_E_L_E_T_=''
			WHERE
			SF3.F3_FILIAL = %xFilial:SF3% 	AND
			SF3.F3_DTCANC = %Exp:Space(8)% 	AND
			SF3.%notdel% 					AND
			SF3.F3_NFISCAL = %Exp:cNota%	AND
			SF3.F3_SERIE = %Exp:cSerie%
		//, cSerie
		) TAB
				...
				If !Empty(aXML[nX][2])
					...
					If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5,7") .And. !cCodRetNFE $ RetCodDene()
						...
						Else
							...
							// BIANCOGRES -> CORRIGE O CAMPO F2_FIMP PARA AS NF'S AUTORIZADAS EM CONTINGENCIA
							If MsSeek(xFilial("SF2")+aNotas[nX][05]+aNotas[nX][04]+aNotas[nX][06]+aNotas[nX][07]) .And. Alltrim(aXML[nX][8]) == "2" .And. SF2->F2_FIMP == "T"
								RecLock("SF2")
								SF2->F2_FIMP := "S"
							EndIf
...
Static Function PrtDanfe(oDanfe,oNFE,cCodAutSef,cModalidade,oNfeDPEC,cCodAutDPEC,cDtHrRecCab,dDtReceb,aNota)
	...
	If Type("oNF:_InfNfe:_infAdic:_infAdFisco:TEXT")<>"U" // BIANCOGRES - Bloco mantido na nova versao para validacao
	...
	If Type("oNF:_InfNfe:_infAdic:_infCpl:TEXT")<>"U" // BIANCOGRES - Bloco mantido na nova versao para validacao
	...
	*/
	...
	//oDanfe:Say(052,098, "Identificação do emitente",oFont12N:oFont) // BIANCOGRES - Comentado
	//nLinCalc	:=	065
	nLinCalc	:=	065 + If (cEmpAnt <> "07", 35, 0) // BIANCOGRES - Comentado acima
	cStrAux		:=	AllTrim(NoChar(oEmitente:_xNome:Text,lConverte))
	//nForTo		:=	Len(cStrAux)/25
	nForTo		:=	Len(cStrAux)/ If (cEmpAnt <> "07", 70, 40) // BIANCOGRES - Comentado acima
	...
	For nX := 1 To nForTo
		...
		oDanfe:Say(nLinCalc,005,SubStr(cStrAux,Iif(nX==1,1,((nX-1)* If (cEmpAnt <> "07", 70, 40))+1), If (cEmpAnt <> "07", 70, 40)), oFont12N:oFont ) // BIANCOGRES - Comentado acima
		//nLinCalc+=10
		nLinCalc += If (cEmpAnt <> "07", 7, 10) // BIANCOGRES - Comentado acima
	Next nX
	...
	nForTo		:=	Len(cStrAux)/70 // BIANCOGRES - Comentado acima
	nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
	For nX := 1 To nForTo
		//oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*40)+1),40),oFont08N:oFont)
		oDanfe:Say(nLinCalc,005,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*70)+1),70),oFont08N:oFont) // BIANCOGRES - Comentado acima
		//nLinCalc+=10
		nLinCalc+=7 // BIANCOGRES - Comentado acima
	Next nX
	...
	If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
		...
		Next nX
		oDanfe:Say(nLinCalc,005, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
		nLinCalc+=10
		oDanfe:Say(nLinCalc,005, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
	Else
		oDanfe:Say(nLinCalc,005, NoChar(oEmitente:_EnderEmit:_xBairro:Text,lConverte)+" Cep:"+TransForm(IIF(Type("oEmitente:_EnderEmit:_Cep")=="U","",oEmitente:_EnderEmit:_Cep:Text),"@r 99999-999"),oFont08N:oFont)
		nLinCalc+=10
		oDanfe:Say(nLinCalc,005, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
		nLinCalc+=10
		oDanfe:Say(nLinCalc,005, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
	...
	// BIANCOGRES - Armazane a Logomarca - INICIO
	//Ticket 11834 - Projeto Consolidacao - Incluida a serie 2 para marca Incesa na empresa 01
	If cEmpAnt == "01" .And. Alltrim(oNFe:_NFE:_INFNFE:_IDE:_SERIE:Text) == "1" //SERIE = 1 => BIANCOGRES
		cLogo := "\SYSTEM\logonfe01.bmp"
	ElseIf cEmpAnt == "01" .And. Alltrim(oNFe:_NFE:_INFNFE:_IDE:_SERIE:Text) == "2" //SERIE = 1 => INCESA NA EMPRESA 01
		cLogo := "\SYSTEM\logonfe05.bmp"
	ElseIf cEmpAnt == "05" .And. Alltrim(oNFe:_NFE:_INFNFE:_IDE:_SERIE:Text) == "1" //SERIE = 1 => INCESA
		cLogo := "\SYSTEM\logonfe05.bmp"
	ElseIf cEmpAnt == "05" .And. Alltrim(oNFe:_NFE:_INFNFE:_IDE:_SERIE:Text) == "2" //SERIE = 2 => BELLACASA
		cLogo := "\SYSTEM\logonfe0599.bmp"
	ElseIf cEmpAnt == "14"
		cLogo := "\SYSTEM\logonfe14.bmp"	
	Else 
		cLogo := "\SYSTEM\logonfe07.bmp"
	EndIf
	// BIANCOGRES - Armazane a Logomarca - FIM
	...
	If nfolha==1
		...
		ElseIf cEmpAnt <> "07"
			...
			oDanfe:SayBitmap(045,005,cLogo,180,045) // BIANCOGRES - Comentado acima
		EndIF
	Endif
	...
	For nY := 1 To nLenItens
		...
		If lflag
			If nL > nMaxItemP2
				...
				oDanfe:Say(010+nLinhavers,098, "Identificação do emitente",oFont12N:oFont) // BIANCOGRES - Comentado
				// Falta corrigir a "Identificação do emitente" para todas as paginas
				...
				Else
					...
					oDanfe:SayBitmap(002,005,cLogo,180,045) // BIANCOGRES - Comentado acima
				EndIf
		...
		If	nL > Iif( (nfolha-1)%2==0 .And. 1==1,MAXITEMP3,nMaxItemP2)
			oDanfe:EndPage()
			oDanfe:StartPage()
			nLenMensagens:= Len(aMensagem)							
...
Static Function GetXML(cIdEnt,aIdNFe,cModalidade)  
	...
	If Empty(cModalidade)    
		...
		oWS:cModelo    := cModel // BIANCOGRES - Bloco mantido na nova versao para validacao
	...
	For nZ := 1 To len(aIdNfe) 
		...
		ConOut("BACP0017 -> fatdanfe -> " + aRetorno[nZ][9] + " - " + aRetorno[nZ][2])
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	For nX := 1 To Len(aCabec)
		...
		aTamCol[nX] := LEN(alltrim(aCabec[nX])) * 5.4
	Next nX
	...
	For nX := 1 To Len(aValores[1])
		...
		For nY := 1 To Len(aValores[1][nX])
			...
			If (LEN(Alltrim(aValores[1][nX][nY])) * 5.4 * nConsTex) > nAux
				...
				nAux := LEN(Alltrim(aValores[1][nX][nY])) * 5.4
