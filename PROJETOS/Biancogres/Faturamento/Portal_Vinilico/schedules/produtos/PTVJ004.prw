...
/*/{Protheus.doc} PTVJ004
Job responsável por sincronizar os produtos
@type function
@version 1.0
@author Pontin - Facile Sistemas
@since 04/09/2020
/*/
User Function PTVJ004(xParam1, xParam2)
  ...
  ElseIf ValType(xParam1) == "C"
    ...
    cFilX		:= xParam2
  Else
    ConOut('## ERROR PTVJ004 - NÃO FOI INFORMADO EMPRESA E FILIAL ##')
    ConOut('## ERROR PTVJ004 - FIM DO JOB ##')
    Return
  EndIf
  ...
  cLockName   := "PTVJ004" + cEmpAnt + cFilAnt

  If !oSchd:BeginMutex( cLockName )

    oSchd:Comunica( " [PTVJ004" + cEmpAnt + cFilAnt + "] Processamento ja iniciado por outra rotina!" )
    RpcClearEnv()
    Return
  ...
  oSchd:cFileLog    := cEmpX + cFilX + "-PTVJ004-" + FwTimeStamp() + ".log"
  ...
  For nI := 1 To Len( aFiliais )
    ...
    //|SINCRONIZA O PRODUTO COM O PORTAL |
    U_PTVPRD01(@oSchd)
