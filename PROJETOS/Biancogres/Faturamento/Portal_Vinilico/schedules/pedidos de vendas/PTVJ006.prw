...
/*/{Protheus.doc} PTVJ006
Job responsável por sincronizar as vendas do Portal com as tabelas de stage area
@type function
@version 1.0
@author Pontin - Facile Sistemas
@since 11/01/2021
/*/
User Function PTVJ006(xParam1, xParam2)
  ...
  ElseIf ValType(xParam1) == "C"
    ...
    cFilX		:= xParam2
  Else
    ConOut('## ERROR PTVJ006 - NÃO FOI INFORMADO EMPRESA E FILIAL ##')
    ConOut('## ERROR PTVJ006 - FIM DO JOB ##')
    Return
  EndIf
  ...
  cLockName   := "PTVJ006" + cEmpAnt + cFilAnt

  If !oSchd:BeginMutex( cLockName )

    oSchd:Comunica( " [PTVJ006" + cEmpAnt + cFilAnt + "] Processamento ja iniciado por outra rotina!" )
    RpcClearEnv()
    Return
  ...
  oSchd:cFileLog    := cEmpX + cFilX + "-PTVJ006-" + FwTimeStamp() + ".log"
