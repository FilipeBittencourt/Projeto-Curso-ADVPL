...
/*/{Protheus.doc} PTVJ007
Job responsável por sincronizar as vendas da stage area com o Protheus
@type function
@version 1.0
@author Pontin - Facile Sistemas
@since 11/01/2021
/*/
User Function PTVJ007(xParam1, xParam2)
  ...
  ElseIf ValType(xParam1) == "C"
    ...
    cFilX		:= xParam2
  Else
    ConOut('## ERROR PTVJ007 - NÃO FOI INFORMADO EMPRESA E FILIAL ##')
    ConOut('## ERROR PTVJ007 - FIM DO JOB ##')
    Return
  EndIf
  ...
  cLockName   := "PTVJ007" + cEmpAnt + cFilAnt

  If !oSchd:BeginMutex( cLockName )

    oSchd:Comunica( " [PTVJ007" + cEmpAnt + cFilAnt + "] Processamento ja iniciado por outra rotina!" )
    RpcClearEnv()
    Return
  ...
  oSchd:cFileLog    := cEmpX + cFilX + "-PTVJ007-" + FwTimeStamp() + ".log"
