...
/*/{Protheus.doc} PTVJ002
Job responsável por enviar os clientes
@type function
@version 1.0
@author Pontin - Facile Sistemas
@since 04/09/2020
/*/
User Function PTVJ002(xParam1, xParam2)
  ...
  ElseIf ValType(xParam1) == "C"
    ...
    cFilX		:= xParam2
  Else
    ConOut('## ERROR PTVJ002 - NÃO FOI INFORMADO EMPRESA E FILIAL ##')
    ConOut('## ERROR PTVJ002 - FIM DO JOB ##')
    Return
  EndIf
  ...
  cLockName   := "PTVJ002" + cEmpAnt + cFilAnt

  If !oSchd:BeginMutex( cLockName )

    oSchd:Comunica( " [PTVJ002" + cEmpAnt + cFilAnt + "] Processamento ja iniciado por outra rotina!" )
    RpcClearEnv()
    Return
  ...
  oSchd:cFileLog    := cEmpX + cFilX + "-PTVJ002-" + FwTimeStamp() + ".log"
