...
/*/{Protheus.doc} PTVJ001
Job responsável por enviar as condições de pagamento
@type function
@version 1.0
@author Pontin - Facile Sistemas
@since 04/09/2020
/*/
User Function PTVJ001(xParam1, xParam2)
  ...
  ElseIf ValType(xParam1) == "C"
    ...
    cFilX		:= xParam2
  Else
    ConOut('## ERROR PTVJ001 - NÃO FOI INFORMADO EMPRESA E FILIAL ##')
    ConOut('## ERROR PTVJ001 - FIM DO JOB ##')
    Return
  EndIf
  ...
  cLockName   := "PTVJ001" + cEmpAnt + cFilAnt

  If !oSchd:BeginMutex( cLockName )

    oSchd:Comunica( " [PTVJ001" + cEmpAnt + cFilAnt + "] Processamento ja iniciado por outra rotina!" )
    RpcClearEnv()
    Return
  ...
  oSchd:cFileLog    := cEmpX + cFilX + "-PTVJ001-" + FwTimeStamp() + ".log"
  ...
  For nI := 1 To Len( aFiliais )
    ...
    //|SINCRONIZA AS CONDIÇÕES DE PAAMENTO COM O PORTAL |
    U_PTVPGT01(@oSchd)
