...
/*/{Protheus.doc} PTVJ003
Job responsável por sincronizar os saldos em estoque
@type function
@version 1.0
@author Pontin - Facile Sistemas
@since 04/09/2020
/*/
User Function PTVJ003(xParam1, xParam2)
  ...
  ElseIf ValType(xParam1) == "C"
    ...
    cFilX		:= xParam2
  Else
    ConOut('## ERROR PTVJ003 - NÃO FOI INFORMADO EMPRESA E FILIAL ##')
    ConOut('## ERROR PTVJ003 - FIM DO JOB ##')
    Return
  EndIf
  ...
  cLockName   := "PTVJ003" + cEmpAnt + cFilAnt

  If !oSchd:BeginMutex( cLockName )

    oSchd:Comunica( " [PTVJ003" + cEmpAnt + cFilAnt + "] Processamento ja iniciado por outra rotina!" )
    RpcClearEnv()
    Return
  ...
  oSchd:cFileLog    := cEmpX + cFilX + "-PTVJ003-" + FwTimeStamp() + ".log"
  ...
  For nI := 1 To Len( aFiliais )
    ...
    //|SINCRONIZA O ESTOQUE COM O PORTAL |
    U_PTVEST01(@oSchd)
