...
USER FUNCTION SOL_INVEST()
	...
	nLinha := 01
	// *****************************  OPCOES DE PESQUISA ***************************************
	OSAY1:= TSAY():NEW(nLinha,01,{|| REPLICATE(" ",14) + "PESQ. com dados em tela" },OSCR,,OBOLD_10,,,,.T.,CLR_WHITE,CLR_BLUE,77,08)
...
Static Function SQL_TODOS(_lDir)
	...
	cSql+= " LEFT JOIN "+RETSQLNAME("SA3")+" SA3 ON SA3.A3_COD	= SZO.ZO_REPRE	AND SA3.D_E_L_E_T_	= '' "+ ENTER
...
Static Function SQL_FILTRO()
	...
	cSql+= " FROM "+RETSQLNAME("SZO")+ " SZO "+ ENTER
	cSql+= " LEFT JOIN "+RETSQLNAME("SA1")+" SA1 ON SZO.ZO_FILIAL	= '01'	AND SA1.A1_COD		= SZO.ZO_CLIENTE AND SA1.A1_LOJA = SZO.ZO_LOJA AND SA1.D_E_L_E_T_ = '' "+ ENTER
	cSql+= " LEFT JOIN "+RETSQLNAME("SA3")+" SA3 ON SA3.A3_COD		= SZO.ZO_REPRE	AND SA3.D_E_L_E_T_	= '' "+ ENTER
	cSql+= " LEFT  JOIN ACY010 ACY ON ACY.ACY_GRPVEN	= A1_GRPVEN		AND ACY.D_E_L_E_T_	= '' "+ ENTER
	cSql+= " WHERE	SZO.ZO_YCOD <> '' "	+ ENTER
	IF ! EMPTY(CREPATU)
