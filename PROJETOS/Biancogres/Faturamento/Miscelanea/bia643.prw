...
Static Function fBIA643D()
	...
	If Empty(_cCodMarc)
		MsgInfo("O preenchimento do campo Marca é Obrigatório!!!")
		Return .F.
	EndIf
...
Static Function fBIA643E()
	...
	If _msCtrlAlt
		...
		// Confirma e gera registros na tabela de Receita Integration para os casos em que os registros não foram incluídos
			...
			If !Empty(_zpTPSEG) .and. _zpTPSEG == GdFieldGet("ZBH_TPSEG",_nI)
				...
				If !Empty(_zpESTADO) .and. _zpESTADO == GdFieldGet("ZBH_ESTADO",_nI)
					...
					If !Empty(_zpPCTGMR) .and. _zpPCTGMR == GdFieldGet("ZBH_PCTGMR",_nI)
						...
						MsgInfo("A chave composta de TipoSegmento / Estado / Pacote GMR só pode existir uma única vez. Na linha: " + Alltrim(Str(_nI)) + " já existe esta chave informada!!!")
						Return .F.
...
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
¦¦+-----------------------------------------------------------------------+¦¦
¦¦¦Funçao    ¦ B643IEXC ¦ Autor ¦ Marcos Alberto S      ¦ Data ¦ 21/06/17 ¦¦¦
¦¦+----------+------------------------------------------------------------¦¦¦
...
User Function B643IEXC()
	...
	AADD(aSays, OemToAnsi("Rotina para importação dos percentuais de Impostos para Orçamento RECEITA."))   
	...
	For _nI	:=	1 to Len(_oGetDados:aCols)
		...
		If !GDdeleted(_nI)
			...
			If xkPosRec <> _nI
				...
				MsgInfo("A chave composta de TpSeg / Estado / Pacote GMR só pode existir uma única vez. Na linha: " + Alltrim(Str(xkPosRec)) + " já existe esta chave informada!!!")
				Return .F.
