...
User Function BIA649()
	...
	If lConfirm
		...
		SL008 += "                       AND ZBL.ZBL_TPSEG = ZBH.ZBH_TPSEG "
			...
			While SL08->(!Eof())
				...
				While SL08->(!Eof()) .and. SL08->EMPR == ksEmpres  
					...
					If lOk
						...
						aadd(oxlVetRec, {"ICMS"     ,"ZBH_VICMS"               ,"1"} )
						aadd(oxlVetRec, {"PIS"      ,"ZBH_VPIS"                ,"1"} )
						...
						For lvxt := 1 to Len(oxlVetRec)
							...
							LV007 += "                               AND ZBL.ZBL_TPSEG = ZBH.ZBH_TPSEG "
							...
							LV007 += "           CLVL, "
							LV007 += "           CONTA "
							U_BIAMsgRun("Aguarde... RECEITA em OrcaFinal " +ksEmpres + " " + AllTrim(Str(lvxt)) ,,{|| msStaExcQy := TcSQLExec(LV007) })
							If msStaExcQy < 0
								lOk := .F.
							EndIf
