...
cDropSC5 := "DROP TABLE IF EXISTS " + nNomeSC5
cDropSC6 := "DROP TABLE IF EXISTS " + nNomeSC6
cDropSC9 := "DROP TABLE IF EXISTS " + nNomeSC9

//Exclui as tabelas temporárias
U_BIAMsgRun("Aguarde...",,{|| TcSQLExec(cDropSC5)})
U_BIAMsgRun("Aguarde...",,{|| TcSQLExec(cDropSC6)})
U_BIAMsgRun("Aguarde...",,{|| TcSQLExec(cDropSC9)})
Return
...
Static Function Monta_Arq()
	...
	Local cSC513 := RetFullName("SC5", "13")
	Local cSC514 := RetFullName("SC5", "14")
	
	Local cSC601 := RetFullName("SC6", "01")
	Local cSC605 := RetFullName("SC6", "05")
	Local cSC613 := RetFullName("SC6", "13")
	Local cSC614 := RetFullName("SC6", "14")
	
	Local cSF401 := RetFullName("SF4", "01")
	Local cSF405 := RetFullName("SF4", "05")
	Local cSF413 := RetFullName("SF4", "13")
	Local cSF414 := RetFullName("SF4", "14")

	Local cSC901 := RetFullName("SC9", "01")
	Local cSC905 := RetFullName("SC9", "05")
	Local cSC913 := RetFullName("SC9", "13")
	Local cSC914 := RetFullName("SC9", "14")
	Local indice := 1

	Local cLinPed := '' //linha descomentada: Pablo S. Nascimento em correção do ticket 22091

	Aadd(empresas, {cSC501, cSC601, "01", cSF401, cSC901})
	Aadd(empresas, {cSC505, cSC605, "05", cSF405, cSC905})
	Aadd(empresas, {cSC513, cSC613, "13", cSF413, cSC913})
	Aadd(empresas, {cSC514, cSC614, "14", cSF414, cSC914})
	...
	If cEmpAnt $ "01_05_07_13_14"  .And. (cEmpAnt+cFilAnt != '0705')

		//Pedidos de cada empresa
		cSql := ""
		cSql += "SELECT * INTO "+nNomeSC5+" FROM (	" + Enter

		For indice := 1 To Len(empresas)

			cSql += "SELECT '"+ empresas[indice][3] +"' AS 'EMPRESA',	" + Enter
			...
			cSql += "FROM "+ empresas[indice][1] +" C5 WITH (NOLOCK) " + Enter
			...
			cSql += "		C5.C5_NUM 		IN (SELECT C6_NUM FROM "+ empresas[indice][2] +" WITH (NOLOCK) WHERE C6_FILIAL = '01' AND C6_NUM = C5.C5_NUM AND (ROUND(C6_QTDVEN,2) - ROUND(C6_QTDENT,2)) > 0 AND C6_BLQ <> 'R' AND D_E_L_E_T_ = '') AND
	...
	If cEmpAnt $ "01_05_07_13_14"  .And. (cEmpAnt+cFilAnt != '0705')
		...
		For indice := 1 To Len(empresas)
			//ITENS DE PEDIDO DE CADA EMPRESA
			cSql += " SELECT '"+ empresas[indice][3] +"' AS EMP, '"+ empresas[indice][3] +"'   AS EMPRESA, C6.C6_FILIAL, C5.C5_NUM, C5.C5_EMISSAO, ISNULL(LC5.C5_CLIENTE, C5.C5_CLIENTE) AS C5_CLIENTE,  C5.C5_VEND1, C6.C6_ITEM,	C6.C6_PRODUTO,	C6_YEMPPED=C5.C5_YEMPPED, " + Enter
			...
			cSql += " FROM "+ empresas[indice][1] +" C5 WITH (NOLOCK) " + Enter
			cSql += " INNER JOIN "+ empresas[indice][2] +" C6 WITH (NOLOCK) ON C5.C5_FILIAL  = C6.C6_FILIAL AND C5.C5_NUM = C6_NUM AND C6.C6_BLQ <> 'R' AND C6.D_E_L_E_T_   = '' " + Enter
			cSql += " INNER JOIN "+ empresas[indice][4] +" F4  WITH (NOLOCK) ON C6.C6_FILIAL = F4.F4_FILIAL AND C6.C6_TES = F4.F4_CODIGO AND F4.D_E_L_E_T_ = '' " + Enter
			...
			//se logado na LM considera as filiais 01 e 05 no select da Bianco 01
			If (cEmpAnt == '07' .And. empresas[indice][3] == "01")
	...
	If cEmpAnt $ "01_05_07_13_14"  .And. (cEmpAnt+cFilAnt != '0705')
		...
		For indice := 1 To Len(empresas)
			...
			cSql += "SELECT '"+ empresas[indice][3] +"' AS 'EMPRESA', C9_FILIAL,C9_PEDIDO,C9_AGREG,C9_PRODUTO,C9_LOTECTL,C9_CLIENTE,C9_LOJA,C9_ITEM,C9_NFISCAL,C9_BLEST,C9_BLCRED,C9_GRUPO,C9_LOCAL,C9_DATALIB,C9_QTDLIB,C9_QTDLIB2, C9_YRASTAT, C9_YDTBLCT, C9_YTPBLCT, C9_YDTLICT " + Enter
			cSql += "FROM "+ empresas[indice][5] +" SC9 WITH (NOLOCK) 		" + Enter
			...
			cSql += "		EMPRESA	   = '"+ empresas[indice][3] +"'			" + Enter
