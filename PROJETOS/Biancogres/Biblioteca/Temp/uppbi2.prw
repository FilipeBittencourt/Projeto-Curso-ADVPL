...
/*/
//--------------------------------------------------------------------
User Function UPPBI2( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPPBI2" )
				Else
					MsgStop( "Atualização não Realizada.", "UPPBI2" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'SA1'																	, .T. }, ; //X3_ARQUIVO
	{ 'Y4'																	, .T. }, ; //X3_ORDEM
	{ 'A1_YLOTRES'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Lote Restr.?'														, .T. }, ; //X3_TITULO
	{ 'Lote Restr.?'														, .T. }, ; //X3_TITSPA
	{ 'Lote Restr.?'														, .T. }, ; //X3_TITENG
	{ 'Aceita Lote Restrito?'												, .T. }, ; //X3_DESCRIC
	{ 'Aceita Lote Restrito?'												, .T. }, ; //X3_DESCSPA
	{ 'Aceita Lote Restrito?'												, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	...
	{ 'Em Foco?'															, .T. }, ; //X3_TITSPA
	{ 'Em Foco?'															, .T. }, ; //X3_TITENG
	{ 'Em Foco?'															, .T. }, ; //X3_DESCRIC
	{ 'Em Foco?'															, .T. }, ; //X3_DESCSPA
	{ 'Em Foco?'															, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ ''																	, .T. }, ; //X3_OBRIGAT
	{ 'Pertence("NE")'														, .T. }, ; //X3_VLDUSER
	{ 'N=Nenhum;E=Engenharia'												, .T. }, ; //X3_CBOX
	{ 'N=Nenhum;E=Engenharia'												, .T. }, ; //X3_CBOXSPA
	{ 'N=Nenhum;E=Engenharia'												, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
	{ ''																	, .T. }, ; //X3_WHEN
	{ ''																	, .T. }, ; //X3_INIBRW
	{ ''																	, .T. }, ; //X3_GRPSXG
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPPBI2" ) ) ) ;
