...
Local _cAliasSr	:=	U_fGetDbSr()
	...
	//RISCO / SOMENTE CLIENTES COM RISCO D
	_cSql := ""
	...
	TcSQLExec(_cSql)
	//CLIENTES DIFERENTE DE FUNCIONARIO
	_cSql := ""
	_cSql += "DELETE ##CLIENTE "
	...
	TcSQLExec(_cSql)
    //CLIENTES DIFERENTE DE REPRESENTANTE
	_cSql := ""
	_cSql += "DELETE ##CLIENTE "
	...
	TcSQLExec(_cSql)
   //CLIENTES COM LIMITE DE CREDITO 
	_cSql := ""
	_cSql += "DELETE ##CLIENTE "
   ...
   //CLIENTES COM ATRASO
	...
	TcSQLExec(_cSql)
    //CLIENTE QUE TIVEREM O CREDITO NEGADO NOS ULTIMOS 30 DIAS
	_cSql := ""
	_cSql += "DELETE ##CLIENTE "
	_cSql += "FROM ##CLIENTE CLI INNER JOIN (SELECT CODIGO = CASE WHEN ZM0_GRUPO = '' THEN 'C-'+ZM0_CLIENT ELSE 'G-'+ZM0_GRUPO END FROM  ZM0010 WHERE ZM0_DATINI >= CONVERT(VARCHAR,DATEADD(DAY,-30,GETDATE()),112) AND ZM0_CREDIT IN ('RM','RA') AND D_E_L_E_T_ = '' ) ZM0 ON CLI.CODIGO = ZM0.CODIGO "
	TcSQLExec(_cSql)
