...
/*/
//--------------------------------------------------------------------
User Function UPD27809( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "PROC2205" )
				Else
					MsgStop( "Atualização não Realizada.", "PROC2205" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 3																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Banco IC'																, .T. }, ; //X3_TITULO
	{ 'Banco IC'																, .T. }, ; //X3_TITSPA
	{ 'Banco IC'																, .T. }, ; //X3_TITENG
	{ 'Banco Intercompany'														, .T. }, ; //X3_DESCRIC
	{ 'Banco Intercompany'													, .T. }, ; //X3_DESCSPA
	{ 'Banco Intercompany'															, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(174) + Chr(255) + Chr(144) + Chr(131) + Chr(244) + ;
	Chr(139) + Chr(240) + Chr(192) + Chr(128) + Chr(128) + ;
...
aAdd( aSX3, { ;
	...
	{ 10																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Agencia IC'															, .T. }, ; //X3_TITULO
	{ 'Agencia IC'															, .T. }, ; //X3_TITSPA
	{ 'Agencia IC'															, .T. }, ; //X3_TITENG
	{ 'Agencia Intercompany'												, .T. }, ; //X3_DESCRIC
	{ 'Agencia Intercompany'												, .T. }, ; //X3_DESCSPA
	{ 'Agencia Intercompany'												, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(174) + Chr(246) + Chr(144) + Chr(131) + Chr(244) + ;
	Chr(137) + Chr(240) + Chr(192) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128)					, .T. }, ; //X3_USADO
	{ ''																	, .T. }, ; //X3_RELACAO
	{ 'BC1'																	, .T. }, ; //X3_F3
	{ 1																		, .T. }, ; //X3_NIVEL
...
aAdd( aSX3, { ;
	...
	{ 10																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Cta Corrente IC'														, .T. }, ; //X3_TITULO
	{ 'Cta Corrente IC'														, .T. }, ; //X3_TITSPA
	{ 'Cta Corrente IC'														, .T. }, ; //X3_TITENG
	{ 'Conta Intercompany'													, .T. }, ; //X3_DESCRIC
	{ 'Conta Intercompany'													, .T. }, ; //X3_DESCSPA
	{ 'Conta Intercompany'													, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(174) + Chr(255) + Chr(144) + Chr(131) + Chr(244) + ;
	Chr(139) + Chr(240) + Chr(192) + Chr(128) + Chr(128) + ;
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Código do banco a ser pago o título.' )

aHlpEng := {}

aHlpSpa := {}

PutSX1Help( "PA2_BANCO  ", aHlpPor, aHlpEng, aHlpSpa, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "A2_BANCO" )

aHlpPor := {}
aAdd( aHlpPor, 'Código da Agencia do Fornecedor.' )
aAdd( aHlpPor, 'Será utlizado quando do bordero de' )
aAdd( aHlpPor, 'pagamento com credito em conta corrente.' )

aHlpEng := {}

aHlpSpa := {}

PutSX1Help( "PA2_AGENCIA", aHlpPor, aHlpEng, aHlpSpa, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "A2_AGENCIA" )

aHlpPor := {}
aAdd( aHlpPor, 'Número da conta corrente do fornecedor.' )
aAdd( aHlpPor, 'Será utilizado quando do borderô com' )
aAdd( aHlpPor, 'crédito em conta corrente.' )

aHlpEng := {}

aHlpSpa := {}

PutSX1Help( "PA2_NUMCON ", aHlpPor, aHlpEng, aHlpSpa, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "A2_NUMCON" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "PROC2205" ) ) ) ;
