...
Method SetContContab(cCad, cCliFor, cLoja, cTipo) Class TContaContabil
	...
	Local cContConta := ""
    Local lIgnoreTipo:=stackTools():IsInCallStack("SetContaContabil")
	...
	If cCad == "C" .And. ((lIgnoreTipo.and.empty(cTipo)).or.cTipo $ MVRECANT)
	...
	If cCad == "F" .And. ((lIgnoreTipo.and.empty(cTipo)).and.cTipo $ MVPAGANT)
...
Method CreateConta(cCad) Class TContaContabil
	...
	If cCad == "C"
        ...
        if (FIDC():isFIDCEnabled())
            aAdd(aConta,{"11201013"+if((Type("M->A1_CONTA")=="C").and.!Empty(M->A1_CONTA),M->A1_CONTA,SA1->A1_COD), "2.01.01.05.01", "2"})
        endif
		...
		For nW := 1 To Len(aConta)
			...
			If CT1->(!MsSeek(xFilial("CT1")+cContConta,.F.))
				...
				if CT1->(RecLock("CT1",.T.))
                    ...
                    If CVN->(MsSeek(xFilial("CVN") + "002   " + cContaRef))
	...
	If cCad == "F"
		...
		For nW := 1 To Len(aConta)
			...
			If CT1->(!MsSeek(xFilial("CT1")+cContConta))
