        ...
        If ::dDtIni > GetMV("MV_DATAFIN") .And. ::dDtFim > GetMV("MV_DATAFIN") // Teoricamente não teria nescessidade de verificar esse parametro por se tratar de comissão. Deixarei por padrão.
            ...
            Begin Transaction
                ...
                While !(cQry)->(Eof())
                    ...
                    If SC5->(DBSeek(xFilial("SC5") + (cQry)->E3_PEDIDO))

                        If SA3->(DBSeek(xFilial("SA3") + (cQry)->E3_VEND))

                            If PZ9->(DBSeek(xFilial("PZ9") + (cQry)->E3_VEND + SC5->C5_YEMP))

                                While !PZ9->(EOF()) .And. PZ9->(PZ9_FILIAL + PZ9_VENDPA + PZ9_MARCA) == xFilial("PZ9") + (cQry)->E3_VEND + SC5->C5_YEMP
                        ...
                        For nX := 1 To Len(aVendRat)
                            ...
                            For nW := 1 To Len(aSE3)
                                ...
                                ElseIf aSE3[nW][1] == "E3_COMIS"
                                    ...
                                    SE3->(&(aSE3[nW][1])) := Round( ( aVendRat[nX][5] * aVendRat[nX][4] ) / 100, 2 )
