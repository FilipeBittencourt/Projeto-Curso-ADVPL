...
Programação idêntica ao FINR190 para preenchimento do Liquidez utilizado no PowerBI        
                                  
/*/
User Function BIAV003(lAuto)
Local oReport	:= Nil   
...
TRFunction():New(oBaixas:Cell("E5_VALOR")		,/*cID*/, "SUM", oBreak1  , STR0071, tm(E5_VALOR,oBaixas:Cell("E5_VALOR"):nSize,nDecs)		, {|| aRelat[nI,11]}	, .F., .F.,.F.) //"Sub Total"
...
If lVarFil .And. !Empty(FWFilial("SE5"))
	...
	TRFunction():New(oBaixas:Cell("E5_VALOR")		,/*[cID*/, "SUM", oBreak2  , STR0070, tm(E5_VALOR,oBaixas:Cell("E5_VALOR"):nSize,nDecs)		, {|| aRelat[nI,11]}	, .F., .F.) //"FILIAL"
...
TRFunction():New(oBaixas:Cell("ABATIMENTO")		,/*[cID*/, "SUM", oBreak3  , "", tm(E5_VALOR,oBaixas:Cell("ABATIMENTO"):nSize,nDecs)	, {|| If(aRelat[nI,26],aRelat[nI,15],0)}						, .F., .F.,.F.)
...
#IFDEF TOP
	If TcSrvType() != "AS/400" .and. TCGetDB()!="SYBASE"
		...
		cQuery += "		  AND E5_TIPODOC NOT IN ('DC','D2','JR','J2','TL','MT','VA','M2','CM','C2','ES'"
		If mv_par11 == 2
			cQuery += " ,'E2'"
		EndIf
		If mv_par16 == 2
		...
		While NEWSE5->(!Eof()) .And. &cCondFil .And. &cCondicao .and. lContinua
			...
			While NEWSE5->(!EOF()) .and. &cCond2=cAnterior .and. &cCondFil .and. lContinua
				...
				If (nDesc+nValor+nJurMul+nVlMovFin) > 0 
					...
					aRet[Li][CATEGORIA] := SA1->A1_YCAT
					aRet[Li][SEGMENTO] := SA1->A1_YTPSEG
					aRet[Li][RISCO] := SA1->A1_RISCO
...
//					If AT("COMPENSAÇÃO NOTA DE CRÉDITO", aRet[li][09]) > 0
//					    dbSelectArea("NEWSE5")
//						NEWSE5->(dbSkip())		      // filtro de registros desnecessarios
//						Loop
//					Endif	
					
//					If AT("Bx.Emis.Fat.", aRet[li][09]) > 0
//					    dbSelectArea("NEWSE5")
//						NEWSE5->(dbSkip())		      // filtro de registros desnecessarios
//						Loop
//					Endif	
//			
					insertDados(aRet[li], mv_par02)
					li++
				Endif
...
Static Function insertDados(aDados, dDataBase)
	...
	If Empty(aDados[DATA_BAIXA]) 
		aDados[DATA_BAIXA] := ""
		cDtBaixa := ""
	Else
		cDtBaixa := DTOS(aDados[DATA_BAIXA]) 
	End If
	
	If Empty(aDados[VL_BAIXA]) 
		vlBaixa := 0
	Else
		vlBaixa := aDados[VL_BAIXA]
	End If
	
		
	cVlBaixa := cValToChar(vlBaixa)
	...
	If !Empty(aDados[CLIENT])
		...
		IF cCodEmp == "01"
			If cCliente != "010064" .and. cCliente != "004536"
				If AT("Bx.Emis.Fat.", cHistorico) == 0 .And. AT("Compensação Nota de Crédito", cHistorico) == 0 .And. AT("Valor Baixado p/Liquidação", cHistorico) == 0
	
			        cSql := "INSERT INTO __BI_PB_CRE  (BK_EMPRESA, BK_FILIAL, BK_CLIENTE, NUM_TIT, NATUREZA, VENCTO, DTBAIXA, SALDO, TOTAL_BAIXADO, VALOR_TOTAL, CATEGORIA, SEGMENTO, RISCO, DATA_BASE, TIPO, HISTORICO, EMPORIGEM, EMPRESA, FILIAL, CLIENTE, MARCA) VALUES"
					cSql += "('P |01|"+cCodEmp+"', 'P |01|"+cCodEmp+cCodFil+"', 'P |01|SA1"+cCodEmp+"0||"+cCliente+"01"
					cSql += "', '"+ cTitulo
					cSql += "', '"+ cNatureza
					cSql += "', '"+ cDtBaixa
					cSql += "', '"+cDtBaixa+"', 0"
					cSql += ", "+cVlBaixa+", "+cVlCorrig
					cSql += ", '"+cCategoria
					cSql += "', '"+cSegmento
					cSql += "', '"+cRisco
					cSql += "', '"+cDataBase+"', "
					cSql += "'BX'," 
					cSql += "'"+cHistorico+"', '"+cCodEmpOri+"', '"+cCodEmp+"', '"+cCodFil+"', '"+cCliente+"', '"+cMarca+"'
					cSql += ")"
					TcSqlExec(cSql)
				End IF
		   End IF
		...
		IF cCodEmp == "05"
			If cCliente != "010064" .and. cCliente != "000481"
				If AT("Bx.Emis.Fat.", cHistorico) == 0 .And. AT("Compensação Nota de Crédito", cHistorico) == 0 .And. AT("Valor Baixado p/Liquidação", cHistorico) == 0
		...
		IF cCodEmp == "07"
			...
			If cCliente != "004536" .and. cCliente != "000481"
				If AT("Bx.Emis.Fat.", cHistorico) == 0 .And. AT("Compensação Nota de Crédito", cHistorico) == 0	 .And. AT("Valor Baixado p/Liquidação", cHistorico) == 0
