...
User Function BIA567()
	...
	Data      := 29/09/2015
	Uso       := Entrega de Pedidos (Comprador - 2 vezes por semana)
	Aplicação := Prazo de entrega de pedidos realizados (Comprador - 2 vezes por semana)
	±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±*/
...
Static Function GeraHtml()
	...
	While !QRY->(EOF())
		...
		cAproAtu := QRY->APROVADOR

		cAproAnt := QRY->APROVADOR

		While !QRY->(EOF()) .And. AllTrim(cAproAnt) == AllTrim(cAproAtu)
			...
			WHILE !QRY->(EOF()) .And. AllTrim(cCForAnt) == AllTrim(cCForAtu) .And. AllTrim(cAproAnt) == AllTrim(cAproAtu)
				...
				If nOpc == ATRASADOS
					...
					If AllTrim(QRY->DT_CHEGADA) <> "" 		
						C_HTML += '    <td><div class="style12">' + Str(DateDiffDay(stod(QRY->DT_CHEGADA), dDataBase)) + ' dia(s) atrasado</div></td> '		
					Else		
				...
				Else
					...
					C_HTML += '    <td><div class="style12">' + SUBSTR(QRY->DT_CHEGADA,7,2)+"/"+SUBSTR(QRY->DT_CHEGADA,5,2)+"/"+SUBSTR(QRY->DT_CHEGADA,1,4) + '</div></td> '
					If AllTrim(QRY->DT_CHEGADA) <> "" 		
						C_HTML += '    <td><div class="style12">' + Str(DateDiffDay(stod(QRY->DT_CHEGADA), dDataBase)) + ' dia(s)</div></td> '		
					Else		
		...
		C_HTML += '<font face = "Arial"> <p>E-mail enviado automaticamente pelo sistema Protheus (by BIA567).</p> </font>    		
		C_HTML += '<p>&nbsp;	</p> '
		C_HTML += '</body> '
		C_HTML += '</html> '
...
Static Function GeraCabItm()   
	...
	C_HTML += '    <th width=7% scope="col"><strong class="style10 style12">Data Previsão</strong></th> '
	C_HTML += '    <th width=7% scope="col"><strong class="style10 style12">Data Chegada</strong></th> '
	If nOpc == ATRASADOS
		C_HTML += '    <th width=8% scope="col"><strong class="style10 style12">Situação</strong></th> '
	Else
		C_HTML += '    <th width=8% scope="col"><strong class="style10 style12">Prazo</strong></th> '
	EndIf
	C_HTML += '  </tr> '
...
Static Function EnvMailMult()
	...
	cRecebe     := UsrRetMail(cAproAnt)	

	//cRecebe 	+=';' 
	If AllTrim(cRecebe) == ''
		cRecebe := 'vagner.salles@biancogres.com.br'
	EndIf
	//cRecebe 	+= U_EmailWF('BIA190',cEmp)
	//Enviar somente empenho da empresa.
	//cRecebe 	+= U_EmailWF('BIA567',cEmpAnt)
	If nOpc == ATRASADOS
...
Static Function MontaQry()
	...
	CSQL +="ORDER BY APROVADOR, SA2.A2_COD, PEDIDO, DT_ENTREGA "
