...
User Function BIABC027()
	Private cEnter := CHR(13)+CHR(10)
	private aPergs := {}
	Private sAnoMes    := AnoMes ( Date() )
	Private oExcel 	
	
    Aviso('Relatório de Compras Realizadas por Departamento', "Compras realizadas do mês de " + MesExtenso(Month2Str( Date() ) ) + "/" + Year2Str( Date() ) ,{'Ok'})
	...
	While !Eof()	
		IncProc()
		oExcel:AddRow(nxPlan, nxTabl, { GU04->DEPART,;
		                                GU04->TIPO,; 
										stod(GU04->EMISSAO),;
										GU04->DOC,;
										GU04->SERIE,; 
...
sQuery += " SELECT DISTINCT ZCA_ENTID CODDEP,						" + cEnter
sQuery += "		   ZCA_DESCRI DEPART,								" + cEnter
sQuery += "		   'NOTA' TIPO,										" + cEnter
sQuery += "		   D1_CLVL CLVL,									" + cEnter
sQuery += "		   D1_GRUPO GRUPO,                      			" + cEnter
...
sQuery += "	   AND ZCN_FILIAL = D1_FILIAL							" + cEnter
sQuery += "	   AND ZCN_LOCAL = D1_LOCAL					           	" + cEnter   
...
sQuery += "	   AND B1_FILIAL = ''				         		    " + cEnter
sQuery += "    AND SB1.D_E_L_E_T_ = ''								" + cEnter
sQuery += "   LEFT JOIN " + RetSQLName("CTH") + " CTH WITH(NOLOCK)  " + cEnter
sQuery += "     ON CTH_CLVL = D1_CLVL								" + cEnter
sQuery += "    AND CTH_FILIAL = ''									" + cEnter
sQuery += "   LEFT JOIN " + RetSQLName("ZCA") + " ZCA WITH(NOLOCK)	" + cEnter
sQuery += "     ON ZCA_ENTID = CTH_YENTID							" + cEnter
sQuery += "	   AND  ZCA.ZCA_FILIAL = ''		       		            " + cEnter
...
sQuery += "	   AND ZCN_FILIAL = D2_FILIAL 							" + cEnter
sQuery += "	   AND ZCN_LOCAL = D2_LOCAL				                " + cEnter
sQuery += "    AND ZCN_POLIT IN ('4','8') 			                " + cEnter   
sQuery += "    AND ZCN_ATIVO <> 'N' 					            " + cEnter   
sQuery += "    AND ZCN.D_E_L_E_T_ = ''				                " + cEnter   
sQuery += "  INNER JOIN " + RetSQLName("SB1") + " SB1  WITH(NOLOCK) " + cEnter
...
sQuery += "    AND SB1.D_E_L_E_T_ = ''								" + cEnter
sQuery += "   LEFT JOIN " + RetSQLName("CTH") + " CTH WITH (NOLOCK) " + cEnter
sQuery += "     ON CTH_CLVL = D2_CLVL								" + cEnter
sQuery += "	   AND CTH_FILIAL = ''									" + cEnter
sQuery += "   LEFT JOIN " + RetSQLName("ZCA") + " ZCA WITH(NOLOCK)	" + cEnter
sQuery += "     ON ZCA_ENTID = CTH_YENTID							" + cEnter
sQuery += "	   AND  ZCA.ZCA_FILIAL = ''				                " + cEnter   
...
sQuery += "    AND SB1.D_E_L_E_T_ = ''								" + cEnter
sQuery += "   LEFT JOIN " + RetSQLName("CTH") + " CTH WITH(NOLOCK)	" + cEnter
sQuery += "     ON CTH_CLVL = C7_CLVL								" + cEnter
sQuery += "	   AND CTH_FILIAL = ''									" + cEnter
sQuery += "   LEFT JOIN " + RetSQLName("ZCA") + " ZCA WITH(NOLOCK)	" + cEnter
sQuery += "     ON ZCA_ENTID = CTH_YENTID							" + cEnter
sQuery += "	   AND  ZCA.ZCA_FILIAL = ''								" + cEnter       
...
sQuery += "  WHERE VALOR <> 0										" + cEnter
sQuery += "  ORDER BY DEPART, TIPO, EMISSAO, DOC, ITEM, GRUPO		" + cEnter 
