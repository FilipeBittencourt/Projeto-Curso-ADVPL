...
±±ºPrograma  ³mt100tok  ºAutor  ³Microsiga           º Data ³  10/29/03   º±±
...
User Function MT100TOK()
	...
	If !FwIsInCallStack('U_GATI001') .Or. (FwIsInCallStack('U_GATI001') .And. !FwIsInCallStack('U_Retorna') .And. !FwIsInCallStack('GeraConhec') .And. !l103Auto)
		...
		/*
		//Retirado a pedido do Sr. Tânia e aprovado pelo Sr Enelcio OS 0175-12
		IF cEmpAnt == '07'
		IF !(ALLTRIM(CA100FOR) $ '000534/ICMS/ICMSTR/PIS/COFINS/IRPJ/CSLL/000399/008070/002912') .AND. CTIPO == 'N'
		MsgBox("Fornecedor digitado não é a Biancogres. Favor procurar o Departamento Contábil","MT100TOK","STOP")
		lret := .F.
		...
		*/
		...
		FOR I := 1 TO LEN(ACOLS)
			...
			IF Subs(CLVL,1,1) = "8" .AND. Alltrim(SF4->F4_ESTOQUE) == "S"
				MsgBox("TES Usada Inválida. Este material é do tipo MD e não deverá atualizar estoque!","MT100TOK","STOP")
				lret := .F.
				TH04->(dbCloseArea())
				...
				IF cREGRA = "N"
					If Alltrim(SBZ->BZ_YMD) == "S"
						...
						Else
							If Alltrim(SF4->F4_ESTOQUE) == "N"
								MsgBox("TES Usada Inválida. Este material é do tipo MD deverá atualizar estoque!","MT100TOK","STOP")
								lret := .F.
								TH04->(dbCloseArea())
								Ferase(TH04+GetDBExtension())
								Ferase(TH04+OrdBagExt())
					...
					ELSEIF Alltrim(SB1->B1_TIPO) == "MP" .OR. Alltrim(SB1->B1_TIPO) == "MC" .OR. Alltrim(SB1->B1_TIPO) == "ME" .OR. Alltrim(SB1->B1_TIPO) == "OI"
						IF Alltrim(SF4->F4_ESTOQUE) <> "S" .And. !_lComum
							MsgBox("TES Usada Inválida. Este material é do tipo MP, MC, ME ou OI e deverá atualizar estoque.","MT100TOK","STOP")
							lret := .F.
							TH04->(dbCloseArea())
							Ferase(TH04+GetDBExtension())
							Ferase(TH04+OrdBagExt())
			...
			If Substr(cCFOP,2,3) == "916"
				If Empty(cNfOri) .or. Empty(cSeriOri) .or. Empty(cItemOri)
					...
					MsgBox("Nota Fiscal de Retorno de Remessa para Conserto." + CHR(13) + CHR(13) + "Necessário verificar se os campos NFOri, SerieOri e ItemOri (bem como o código do produto/fornecedor/loja) estão devidamente preenchidos!!!","MT100TOK","ALERT")
					lret := .F.
					TH04->(dbCloseArea())
					Ferase(TH04+GetDBExtension())
					Ferase(TH04+OrdBagExt())
				...
				Else
					...
					dbGoTop()
					If YT06->SALDO < 0
						MsgBox("Nota Fiscal de Retorno de Remessa para Conserto." + CHR(13) + CHR(13) + "Necessário verificar se os campos NFOri, SerieOri e ItemOri estão devidamente preenchidos, pois não há saldo suficiente para atender esta devolução!!!","MT100TOK","STOP")
						lret := .F.
						YT06->(dbCloseArea())
			...
			// Por Marcos Alberto Soprani em 26/09/14, conforme descrito acima na declaração da variável.
			...
			If INCLUI .OR. ALTERA
				lret := U_FOPVCR01(I)
				If !lret
					MsgBox("Não foi encontrada OP com saldo disponível para entrada deste Item/NF, verificar com o Setor de PCP.","MT100TOK","STOP")
					TH04->(dbCloseArea())
					Ferase(TH04+GetDBExtension())
					Ferase(TH04+OrdBagExt())
					RestArea(aArea)
			...
			//|Pontin / Facile - Validação de pedidos de compras com forma de pagamento diferentes na mesma NF |
		...
		While !Eof()
			...
			If RV03->CONTAD <> 0
				...
				MsgBox("O IDENT: " + TH04->IDENTB6 + " informado não tem quantidade suficiente para atender às linhas onde ele foi informado. Favor verificar...","MT100TOK","STOP")
				TH04->(dbCloseArea())
				Ferase(TH04+GetDBExtension())
				Ferase(TH04+OrdBagExt())
		...
		IF SM0->M0_CODIGO <> '02'
			...
			If nSitNat == "1"
				MsgBox("Natureza Financeira "+Alltrim(SED->ED_CODIGO)+" está bloqueada para uso!","MT100TOK","STOP")
				Return(.F.)
			EndIf
		Endif
		...
		IF ALLTRIM(CTIPO) = "D" .AND. ALLTRIM(_TIPO_SB1) = "PA"
			...
			C_TITULO 	:= "Nota fiscal de devolução"			
			C_DESTI		:= "nadine.araujo@biancogres.com.br"

			//C_MENS 		:= "Foi lançada uma nota fiscal de devolução na " + IIF(CEMPANT="01","Biancogres",IIF(CEMPANT="05","Incesa","Biancogres"))
			//C_MENS 		+= " "  + CHR(13)+CHR(10)
			//C_MENS 		+= "Nota Fiscal:	" 	+	CNFISCAL + CHR(13)+CHR(10)
			//C_MENS 		+= "Cod Cliente:	" 	+ 	CSERIE + CHR(13)+CHR(10)
			//C_MENS 		+= "Emissaão:	" 			+ 	DTOC(DDEMISSAO) + CHR(13)+CHR(10)
		...
		// Incluído por Marcos Alberto Soprani em 19/07/12 conforme OS Effettivo 0677-12
		if Type("oFisRod") <> "U"
			For np := 1 to Len(oFisRod:AARRAY)
				If Alltrim(oFisRod:AARRAY[np][1]) == "IRR"
					If Empty(cCodRet) .or. cDirf == "2"
						MsgBox("Esta nota possui Imposto de Renda e é obrigatório a digitação do Código de Retenção.","MT100TOK","STOP")
						lRet := .F.
					EndIf
				EndIf
				If Alltrim(oFisRod:AARRAY[np][1]) == "PIS"
					If Empty(cCodRet) .or. cDirf == "2"
						MsgBox("Esta nota possui PIS e é obrigatório a digitação do Código de Retenção.","MT100TOK","STOP")
						lRet := .F.
					EndIf
				EndIf
				If Alltrim(oFisRod:AARRAY[np][1]) == "COF"
					If Empty(cCodRet) .or. cDirf == "2"
						MsgBox("Esta nota possui COFINS e é obrigatório a digitação do Código de Retenção.","MT100TOK","STOP")
						lRet := .F.
					EndIf
				EndIf
				If Alltrim(oFisRod:AARRAY[np][1]) == "CSL"
					If Empty(cCodRet) .or. cDirf == "2"
						MsgBox("Esta nota possui CSLL e é obrigatório a digitação do Código de Retenção.","MT100TOK","STOP")
						lRet := .F.
					EndIf
				EndIf
			Next np
		...
		If cFormul == "N"
			If Alltrim(cEspecie) $ "SPED/CTE/CTEOS" .and. Empty(M->F1_CHVNFE) .and. 1 == 2 // retirado dia 28/01/2016 O.S 0260-16 4444-15
				MsgBox("Se a ESPECIE for SPED/CTE/CTEOS a chave deverá ser informada.","MT100TOK","STOP")
				lRet := .F.
			Else
				If Alltrim(cEspecie) $ "SPED/CTE/CTEOS" .and. INCLUI
					...
					dbSelectArea("TX01")
					If TX01->CONTAD > 0
						MsgBox("Chave eletrônica duplicada. Será necessário filtrar, na tabela SF1, quais notas estão associadas com a chave acima e efetuar o devido acerto.","MT100TOK","STOP")
						lRet := .F.
						lContinua := .F.
					EndIf
					TX01->(dbCloseArea())
					...
					If lContinua
						...
						If Substr(M->F1_CHVNFE,7,14) <> xfCnpj .And. Substr(M->F1_CHVNFE,7,14) <> "83817858007183" .And. Substr(M->F1_CHVNFE,7,14) <> "08197731001233"
							MsgBox("A chave eletrônica desta nota não pertence ao fornecedor corrente. Favor verificar!","MT100TOK","STOP")
							lRet := .F.
							lContinua := .F.
						EndIf
					...
					If lContinua
						If Substr(M->F1_CHVNFE,26,9) <> CNFISCAL
							MsgBox("A chave eletrônica Informada não pertence ao Numero de Nota Corrente. Favor verificar!","MT100TOK","STOP")
							lRet := .F.
						EndIf
					EndIf
				EndIf
				If lContinua
					...
					If !Empty(M->F1_CHVNFE) .and. !Alltrim(cEspecie) $ "SPED/CTE/CTEOS"
						MsgBox("O Campo Chave Eletrônica foi informado portanto é necessário informar o ESPECIE como SPED, CTE ou CTEOS","MT100TOK","STOP")
						lRet := .F.
					EndIf
				EndIf
			EndIf
		...
		// Por Marcos Alberto Soprani em 20/09/12 comforme OS Effettivo 2114-12
		// Foi necessário efetuar este tratamento porque quando o retorno já está com problema em uma das regras e entra ne regra abaixo, se confirmada passa. Por Marcos Alberto Soprani em 02/10/12
		// 19/06 => Portaria Fiscal / Fernando  / Não executar se for execauto de classificacao
		If lRet .And. !IsInCallStack("U_TACLNFJB") .And. !IsInCallStack("U_BACP0012") .And. !IsInCallStack("U_PNFM0002") .And. !IsInCallStack("U_PNFM0005") .And. !IsInCallStack("U_JOBFATPARTE")
			If CtnSrvc
				lRet := MsgNOYES("Esta nota contém ITENS de SERVIÇO. Necessário certificar-se de que o valor do ISS esteja informado corretamente."+CHR(13)+CHR(13)+"Confirma a inclusão da nota, certo de que o valor do ISS está correto?","Atenção. (MT100TOK)")
			Else
				If Type("oFisRod") <> "U"
					xFoldImp := oFisRod:AARRAY
					For yg := 1 to Len(xFoldImp)
						If Alltrim(oFisRod:AARRAY[yg][1]) == "ISS"
							lRet := MsgNOYES("Esta nota NÃO contém ITENS de SERVIÇO. Necessário certificar-se de que o valor do ISS esteja informado corretamente."+CHR(13)+CHR(13)+"Confirma a inclusão da nota, certo de que o valor do ISS está correto?","Atenção. (MT100TOK)")
						EndIf
					Next yg
				EndIf
			EndIf
		...
		If cFormul == "N"
			If Len(Alltrim(cNFiscal)) <> 9 .or. cNFiscal == "000000000"
				MsgBox("Problema com a nuneração do documento fiscal. Favor verificar!!!","MT100TOK","STOP")
				lRet := .F.
			EndIf
		EndIf
		...
		If Alltrim(cEspecie) $ "CTE/CTEOS"
			For wr := 1 to 3
				If !Substr(cSerie,wr,1) $ "1_2_3_4_5_6_7_8_9_0_ "
					MsgBox("Quando se tratar de CTE/CTEOS não é permitida a digitação da SERIE com LETRAS. Favor verificar!!!","MT100TOK","STOP")
					lRet := .F.
					Exit
				EndIf
			Next wr
		...
		If !cTipo $ "D/B" .and. Alltrim(cEspecie) <> "RPS"
			...
			If !Empty(kwCodEmp)
				...
				If RT05->CONTAD == 0 .And. Upper(AllTrim(getenvserver())) == "PRODUCAO" //Executa validacao somente para ambiente de PRODUCAO 
					MsgBox("Nota fiscal emitada INTRAGRUPO não localizada na empresa ORIGEM! Será necessário verificar a numeração desta nota fiscal na empresa ORIGEM antes de efetuar o lançamento desta nota nesta empresa.","MT100TOK","STOP")
					lRet := .F.
				EndIf
				RT05->(dbCloseArea())
		...
		If dDataBase <= GetMv("MV_YULMES")
			MsgBox("Impossível prosseguir, pois este movimento interfere no fechamento de custo!!! Favor verificar com a contabilidade!!!","MT100TOK","STOP")
			lRet := .F.
		EndIf
		...
		If lRet .And. Len(aItemPC) > 0
			...
			*/
			...
			If !Empty(cMsgPC) .And. !isInCallStack("U_TACLNFJB") .And. !IsInCallStack("U_PNFM0002") .And. !IsInCallStack("U_PNFM0005") .And. IsInCallStack("U_JOBFATPARTE")
				...
				psOpc	:= Aviso("MT100TOK", "Existem itens com divergência na condição de pagamento vinculada ao Pedido de Compras!!" + Chr(13) + Chr(10) + Chr(13) + Chr(10) +;
				"Segue abaixo detalhes dos itens:" + Chr(13) + Chr(10) + cMsgPC, {"Continuar","Voltar"}, 3)
				If psOpc == 1
					If !MsgNoYes("Tem certeza que não seria melhor resolver o problema antes de prosseguir?", "Divergência na condição de pagamento(1)" )
						lRet	:= .F.
					Else
						If !MsgYesNO("Realmente deseja continuar antes de avaliar? Tudo bem então!!! Não poderei fazer nada para ajudá-lo!!! Ok?", "Divergência na condição de pagamento(2)" )
							lRet	:= .F.
						EndIf
					EndIf
				Else
		...
		//Projeto contrato de verbas - validação de saldo do contrato para notas de bonificação.
			...
			if len(aContrato) > 0
				For np := 1 to Len(aContrato) 
					...
					if aContrato[np,2] > nSaldCtr
						lRet := .F.
						MsgBox("O contrato numero "+aContrato[np,1]+" não possui saldo suficiente para esta Bonificação! ";
			...
			For np := 1 to Len(aCols) 
				if !GdDeleted(np) .and. Alltrim(GdFieldGet("D1_CONTA",np)) <> "41301001" //.and. !Empty(Gdfieldget('D1_CONTA',np)) .And. Substr(Gdfieldget('D1_CONTA',np),1,1) == '6' .And. !Empty(Gdfieldget('D1_CLVL',np)) 
					...
					If CTH->(DbSeek(xFilial("CTH")+Gdfieldget('D1_CLVL',np))) .And. CTH->CTH_YATRIB == 'C' .And. Gdfieldget('D1_YAPLIC',np) $ ' _0'
						MsgBox("A Conta e a Classe de valor exigem que a Aplicação seja Informada e diferente de zero!","MT100TOK","STOP")
						lRet	:=	.F.
						Exit
					endif
				endif
...
Static Function ATU_PROCEX()
	...
	NEXT
	dbcommitall()
	MsgBox("Este Lancamento foi registrado para o processo de Exportação No: "+M->E2_YPROCEX,"MT100TOK","STOP")
	...
	Else
		...
		If !cTrab->(EOF())
			...
			"Esta despesa ficará bloqueada para baixa.","MT100TOK","STOP")
			DbSelectArea("cTrab")
			DbCloseArea()
			Return
		Else
...
User Function NDV_MAIL(_CEMIS,_CHORA,_CCLI,_CNUMENT,_CSERENT,_CNUMERO,_CSERIE,_CPEDIDO,_CREPRES)
	...
	IF CEMPANT = "05"
		_CEMPRESA += 'INCESA REVESTIMENTO CERÂMICO LTDA'
	ELSE
	...
	_CMENS+='     <td width="189" class="style12"><div align="right"> DATA EMISSÃO: '+_CEMIS+' </div></td> '
	_CMENS+='   </tr> '
	_CMENS+='   <tr> '
	...
	_CMENS+='   </tr> '
	_CMENS+=' </table> '
	_CMENS+=' <p class="style35">Esta é uma mensagem automática, favor não responde-la. (Fonte/Função: MT100TOK.prw/ATU_PROCEX) </p> '
	_CMENS+=' </body> '
	_CMENS+=' </html> '
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³fValidCHV   Autor ³ Thiago Haagensen    º Data ³  13/11/20  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ Valida chave da nota fiscal se ja existe no banco de dados º±±	 
º±± 	  																  º±±			  
±±º			   						  									  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ Compras		                                          	  º±±
...
If !(ALLTRIM(FUNNAME())$ "MATA920/SPEDNFE") //Ticket 28448 - Adicionada as exceções para MATA920/SPEDNFE
	...
	If !Empty(cChvNFEAux)
		Aviso("Nota Fiscal Eletronica", "Atenção ! Esta chave que está sendo inserida já existe para NF: "+cDocumento + " Série : "+cSerieC+".",{"Fechar"},2)
		lRet:=.F.
	Return lRet
	Else
		lRet:=.T.
