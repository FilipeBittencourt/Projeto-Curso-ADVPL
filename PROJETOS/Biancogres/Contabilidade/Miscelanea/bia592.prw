...
@since 02/11/17
@version 1.0
@description Browser principal para a rotina de Depreciação (CAPEX e Ativo Fixo) para OrcaFinal
@type function
/*/
...
User Function BIA592()
	...
	Private cCadastro 	:= "Depreciação (CAPEX e Ativo Fixo) p/ OrcaFinal"

	Private aRotina 	:= { {"Pesquisar"  			          ,"AxPesqui"     ,0,1},;
	{                         "Visualizar"			          ,"AxVisual"     ,0,2},;
	{                         "Depreciação p/ OrcaFinal"      ,"U_B592IMDD"   ,0,3} }
...
User Function B592IMDD()
	...
	AADD(aSays, OemToAnsi("Rotina para Integração dos registros de Depreciação (CAPEX e Ativo Fixo) p/ OrcaFinal!"))   
	...
	FormBatch( OemToAnsi('Integração Depreciação p/ OrcaFinal'), aSays, aButtons ,,,500)
	...
	If lConfirm
		...
		If M007->CONTAD <> 0
			...
			xkContinua := MsgNOYES("Já existe base CAPEX para a Versão / Revisão / AnoRef informados." + msrhEnter + msrhEnter + " Importante: caso confirme, o sistema irá efetuar a limpeza dos dados gravados." + msrhEnter + msrhEnter+ " Deseja prosseguir com o reprocessamento!!!")
...
Static Function fPergIntMD()
	...
	If ParamBox(aPergs ,"Integração Depreciação p/ OrcaFinal",,,,,,,,cLoad,.T.,.T.)      
...
Static Function fProcIntMD()
	...
	Begin Transaction
		...
		If lOk
			...
			For lvxt := 1 to 12
				...
				LV007 += "    AND MESREF <> 0 "
				U_BIAMsgRun("Aguarde... Convertendo CAPEX em OrcaFinal... ",,{|| msStaExcQy := TcSQLExec(LV007) })
	...
	If lOk
		...
		MsgINFO("Conversão Depreciação (CAPEX e Ativo Fixo) p/ OrcaFinal realizada com sucesso!!!", "BIA592")
