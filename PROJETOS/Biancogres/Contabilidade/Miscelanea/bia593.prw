...
@since 28/09/17
@version 1.0
@description Tela para lançamento das ajustes orçamentário na tabela geral de Orçamento ZBZ 
@type function
/*/

User Function BIA593()
	...
	aAdd(_aButtons,{"PEDIDO"  ,{|| U_B593IEXC() }, "Importa Arquivo"   , "Importa Arquivo"})
	...
	@ 048,050 MSGET _oGVersao VAR _cVersao Picture "@!" F3 "ZB5" SIZE 50, 11 OF _oDlg PIXEL VALID fBIA593A()

	@ 050,110 SAY "Revisão:" SIZE 55, 11 OF _oDlg PIXEL FONT oFont
	@ 048,150 MSGET _oGRevisa VAR _cRevisa  SIZE 50, 11 OF _oDlg PIXEL VALID fBIA593B()

	@ 050,210 SAY "AnoRef:" SIZE 55, 11 OF _oDlg PIXEL FONT oFont
	@ 048,250 MSGET _oGAnoRef VAR _cAnoRef  SIZE 50, 11 OF _oDlg PIXEL VALID fBIA593C()

	@ 050,310 SAY "DataRef:" SIZE 55, 11 OF _oDlg PIXEL FONT oFont
	@ 048,350 MSGET _oGDataRef VAR _cDataRef  SIZE 50, 11 OF _oDlg PIXEL VALID fBIA593D()

	@ 050,410 SAY "HistFiltro:" SIZE 55, 11 OF _oDlg PIXEL FONT oFont
	@ 048,450 MSGET _oGHistFil VAR _cHistFil  SIZE 100, 11 OF _oDlg PIXEL VALID fBIA593G()

	_oGetDados := MsNewGetDados():New(_aPosObj[2,1], _aPosObj[2,2], _aPosObj[2,3], _aPosObj[2,4], 7, "U_B593LOK()" /*[ cLinhaOk]*/, /*[ cTudoOk]*/, "+++ZBZ_LINHA" /*[ cIniCpos]*/, /*Acpos*/, /*[ nFreeze]*/, 99999 /*[ nMax]*/, "U_B593FOK()" /*cFieldOK*/, /*[ cSuperDel]*/,"U_B593DOK()" /*[ cDelOk]*/, _oDlg, _aHeader, _aCols)
...
Static Function fBIA593A()
	...
	If !MsgYesNo("Deseja filtrar por data antes de prosseguir?", "Atenção")
		If !Empty(_cVersao) .and. !Empty(_cRevisa) .and. !Empty(_cAnoRef)
			...
			Processa({ || cMsg := fBIA593F() }, "Aguarde...", "Carregando dados...",.F.)
		EndIf
	EndIf

Return .T.

Static Function fBIA593B()
...
Static Function fBIA593F()
	...
	If Empty(_cDataRef)
		...
		Else
			...
			_cLkHistFil	:= "% ZBZ_HIST like '%"+AllTrim(_cHistFil)+"%' %"
		...
		While (_cAlias)->(!Eof())
			...
			For _msc := 1 to Len(_oGetDados:aHeader)
				...
				ElseIf Alltrim(_oGetDados:aHeader[_msc][2]) == "ZBZ_DDEB"
					_oGetDados:aCols[Len(_oGetDados:aCols), _msc] := Posicione("CT1", 1, xFilial("CT1") + (_cAlias)->ZBZ_DEBITO, "CT1_DESC01")

				ElseIf Alltrim(_oGetDados:aHeader[_msc][2]) == "ZBZ_DCRD"
					_oGetDados:aCols[Len(_oGetDados:aCols), _msc] := Posicione("CT1", 1, xFilial("CT1") + (_cAlias)->ZBZ_CREDIT, "CT1_DESC01")

				ElseIf Alltrim(_oGetDados:aHeader[_msc][2]) == "ZBZ_DCVDB"
					_oGetDados:aCols[Len(_oGetDados:aCols), _msc] := Posicione("CTH", 1, xFilial("CTH") + (_cAlias)->ZBZ_CLVLDB, "CTH_DESC01")

				ElseIf Alltrim(_oGetDados:aHeader[_msc][2]) == "ZBZ_DCVCR"
					_oGetDados:aCols[Len(_oGetDados:aCols), _msc] := Posicione("CTH", 1, xFilial("CTH") + (_cAlias)->ZBZ_CLVLCR, "CTH_DESC01")

				ElseIf Alltrim(_oGetDados:aHeader[_msc][2]) == "ZBZ_DATA"
					_oGetDados:aCols[Len(_oGetDados:aCols), _msc] := stod((_cAlias)->ZBZ_DATA)

				ElseIf Alltrim(_oGetDados:aHeader[_msc][2]) == "ZBZ_YDELTA"
					_oGetDados:aCols[Len(_oGetDados:aCols), _msc] := stod((_cAlias)->ZBZ_YDELTA)
...
User Function B593FOK()
	...
	If !GDdeleted(_nAt)
		...
		Do Case
			...
			isCLVLDB   := GdFieldGet("ZBZ_CLVLDB",_nAt)
			isCLVLCR   := GdFieldGet("ZBZ_CLVLCR",_nAt)
...
User Function B593LOK()

	Local _lRet	:=	.T.
	xxDC       := GdFieldGet("ZBZ_DC", n)
	xxDEBITO   := GdFieldGet("ZBZ_DEBITO", n)
	xxCREDIT   := GdFieldGet("ZBZ_CREDIT", n)
	xxCLVLDB   := GdFieldGet("ZBZ_CLVLDB", n)
	xxCLVLCR   := GdFieldGet("ZBZ_CLVLCR", n)
	xxITEMD    := GdFieldGet("ZBZ_ITEMD", n)
...
User Function B593DOK()

	Local _lRet	:=	.T.

Return _lRet

User Function B593VdCl(ksCLVL)
...
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
¦¦+-----------------------------------------------------------------------+¦¦
¦¦¦Funçao    ¦ B593IEXC ¦ Autor ¦ Marcos Alberto S      ¦ Data ¦ 21/06/17 ¦¦¦
¦¦+----------+------------------------------------------------------------¦¦¦
¦¦¦Descriçào ¦ Importação planilha Excel para Orçamento - Custo Variável  ¦¦¦
¦¦+-----------------------------------------------------------------------+¦¦
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯*/
User Function B593IEXC()
	...
	AADD(aSays, OemToAnsi("Rotina para importação dos ajustes orçamentário direto para a tabela ZBZ."))   
	...
	If Len(aArquivo) > 0 
		...
		For nx := 1 to len(aWorksheet) 
			...
			Else
				...
				nLinReg   := 0
				nPosRec   := aScan(aCampos,{|x| AllTrim(x) == "ZBZ_REC_WT"})
