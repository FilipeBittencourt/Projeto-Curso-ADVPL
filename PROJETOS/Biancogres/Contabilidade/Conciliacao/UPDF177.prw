...
/*/
//--------------------------------------------------------------------
User Function UPDF177( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDF177" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDF177" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX2, { ;
	'ZKU'																	, ; //X2_CHAVE
	cPath																	, ; //X2_PATH
	'ZKU'+cEmpr																, ; //X2_ARQUIVO
	'Saldo Financeiro Cliente'												, ; //X2_NOME
	'Saldo Financeiro Cliente'												, ; //X2_NOMESPA
	'Saldo Financeiro Cliente'												, ; //X2_NOMEENG
	'E'																		, ; //X2_MODO
	''																		, ; //X2_TTS
	''																		, ; //X2_ROTINA
	''																		, ; //X2_PYME
...
aAdd( aSX3, { ;
	...
	{ 'Conta Conabil'														, .T. }, ; //X3_DESCRIC
	{ 'Conta Conabil'														, .T. }, ; //X3_DESCSPA
	{ 'Conta Conabil'														, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	...
	{ 'R'																	, .T. }, ; //X3_CONTEXT
	{ '€'																	, .T. }, ; //X3_OBRIGAT
	{ 'ExistChav("ZKU", M->ZKU_CONTA)'										, .T. }, ; //X3_VLDUSER
	{ ''																	, .T. }, ; //X3_CBOX
	{ ''																	, .T. }, ; //X3_CBOXSPA
	{ ''																	, .T. }, ; //X3_CBOXENG
	{ ''																	, .T. }, ; //X3_PICTVAR
...
aAdd( aSX3, { ;
	{ 'ZKU'																	, .T. }, ; //X3_ARQUIVO
	{ '03'																	, .T. }, ; //X3_ORDEM
	{ 'ZKU_SALDO'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 12																	, .T. }, ; //X3_TAMANHO
	{ 2																		, .T. }, ; //X3_DECIMAL
	{ 'Saldo Ajuste'														, .T. }, ; //X3_TITULO
	{ 'Saldo Ajuste'														, .T. }, ; //X3_TITSPA
	{ 'Saldo Ajuste'														, .T. }, ; //X3_TITENG
	{ 'Saldo Ajuste'														, .T. }, ; //X3_DESCRIC
	{ 'Saldo Ajuste'														, .T. }, ; //X3_DESCSPA
	{ 'Saldo Ajuste'														, .T. }, ; //X3_DESCENG
...
aAdd( aSX3, { ;
	...
	{ 8																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Data Ajuste'															, .T. }, ; //X3_TITULO
	{ 'Data Ajuste'															, .T. }, ; //X3_TITSPA
	{ 'Data Ajuste'															, .T. }, ; //X3_TITENG
	{ 'Data Ajuste'															, .T. }, ; //X3_DESCRIC
	{ 'Data Ajuste'															, .T. }, ; //X3_DESCSPA
	{ 'Data Ajuste'															, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
aAdd( aSIX, { ;
	'ZKU'																	, ; //INDICE
	'1'																		, ; //ORDEM
	'ZKU_FILIAL+ZKU_CONTA'													, ; //CHAVE
	'C. Contabil'															, ; //DESCRICAO
	'C. Contabil'															, ; //DESCSPA
	'C. Contabil'															, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDF177" ) ) ) ;
