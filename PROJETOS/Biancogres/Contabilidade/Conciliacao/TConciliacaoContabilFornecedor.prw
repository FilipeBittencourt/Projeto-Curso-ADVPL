...
/*/{Protheus.doc} TConciliacaoContabilFornecedor
@author Tiago Rossini Coradini
@since 09/01/2020
@version 1.0
@description Conciliacao Contabil - Fornecedor 
@obs Projeto: A-54
@type class
/*/

Class TConciliacaoContabilFornecedor From LongClassName 
...
Method New(oParam) Class TConciliacaoContabilFornecedor
...
Method Export() Class TConciliacaoContabilFornecedor
Local aArea := GetArea()
Local oFWExcel := Nil
Local oMsExcel := Nil
Local cDir := GetSrvProfString("Startpath", "")
Local cFile := "BIAF165-" + cEmpAnt + __cUserID + "-" + dToS(Date()) +"-"+ StrTran(Time(), ":", "") + ".XML"
...
Local cQry := GetNextAlias()
	...
	TcSPExec("SP_RELPOSCLI_CP_ANALITICO", dToS(dData), cCodFor, cCodFor)
	
	cSQL := " SELECT SUM(SALDO) AS SALDO "
	cSQL += " FROM POSCLI_DATA_CP_ANALITICO "
	cSQL += " WHERE EMPFIL = " + ValToSQL(cEmpAnt + cFilAnt)
	cSQL += " AND DATAREF = " + ValToSQL(dData)
	cSQL += " AND CODFOR = " + ValToSQL(cCodFor)
...
Method GetSalCon(dData, cConta) Class TConciliacaoContabilFornecedor
Local nRet := 0

	nRet := SaldoConta(cConta, dData, "01")
