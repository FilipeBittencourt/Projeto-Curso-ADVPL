...
@since 28/09/17
@version 1.0
@description Tela para cadastro do Índice de Variação da Quantidade da Pre-Estrutura 
@type function
/*/
...
User Function BIA630()
	...
	Define MsDialog _oDlg Title "Ajustes de Quantidade para Custo Variável" From _aSize[7],0 To _aSize[6],_aSize[5] Of oMainWnd Pixel
...
¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦¦
¦¦+-----------------------------------------------------------------------+¦¦
¦¦¦Funçao    ¦ B630IEXC ¦ Autor ¦ Marcos Alberto S      ¦ Data ¦ 21/06/17 ¦¦¦
¦¦+----------+------------------------------------------------------------¦¦¦
				...
				If nPosRec <> 0
					...
					If nLinReg == 0 .or. Val(Alltrim(aLinha[nPosRec])) == 0
						...
						If nLinChave <> 0
							...
							MsgINFO("O chave [produto + componente] oriundo do excel, já existe na tela. Linha: " + Alltrim(Str(nLinChave)) + " :[ " + aLinha[vtCODPos] + " || " + aLinha[vtCOMPPos] + " ]. O registro será desconsiderado. Atenciosamente!!!")
...
User Function B630EXCL()
	...
	If !_msCtrlAlt  
		MsgInfo("Não é permitido alterar dados porque a Versão orçamentária está bloquada.")
	Else
		If MsgNoYes("A opção escolhida marcará os registros carregados para exclusão e será completada somente após clicar em salvar na janela principal. Deseja Prosseguir?","Exclusão de Registros")
