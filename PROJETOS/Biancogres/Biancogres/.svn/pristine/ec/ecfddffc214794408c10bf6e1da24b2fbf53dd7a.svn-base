...
/*/
//--------------------------------------------------------------------
User Function UPD13639( cEmpAmb, cFilAmb )
...
Local   cDesc6    := ""
Local   cDesc7    := "Compatibilizador referente ao ticket: 13639 - Inserir regra para classificação de clientes na perda"
Local   cMsg      := ""
Local   lOk       := .F.
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDVO" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDVO" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX2, { ;
	...
	cPath																	, ; //X2_PATH
	'ZAJ010'																, ; //X2_ARQUIVO
	'HISTORICO CLASSIFICACAO CLIENT'										, ; //X2_NOME
	'HISTORICO CLASSIFICACAO CLIENT'										, ; //X2_NOMESPA
	'HISTORICO CLASSIFICACAO CLIENT'										, ; //X2_NOMEENG
	'C'																		, ; //X2_MODO
	''																		, ; //X2_TTS
	''																		, ; //X2_ROTINA
	''																		, ; //X2_PYME
...
aAdd( aSX3, { ;
	{ 'ZAJ'																	, .T. }, ; //X3_ARQUIVO
	{ '05'																	, .T. }, ; //X3_ORDEM
	{ 'ZAJ_SITANT'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Sit Anterior'														, .T. }, ; //X3_TITULO
	{ 'Sit Anterior'														, .T. }, ; //X3_TITSPA
	{ 'Sit Anterior'														, .T. }, ; //X3_TITENG
	{ ''																	, .T. }, ; //X3_DESCRIC
	{ ''																	, .T. }, ; //X3_DESCSPA
	{ ''																	, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
...
aAdd( aSX3, { ;
	{ 'ZAJ'																	, .T. }, ; //X3_ARQUIVO
	{ '06'																	, .T. }, ; //X3_ORDEM
	{ 'ZAJ_SITATU'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 1																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Sit Atual'															, .T. }, ; //X3_TITULO
	{ 'Sit Atual'															, .T. }, ; //X3_TITSPA
	{ 'Sit Atual'															, .T. }, ; //X3_TITENG
	{ ''																	, .T. }, ; //X3_DESCRIC
	{ ''																	, .T. }, ; //X3_DESCSPA
	{ ''																	, .T. }, ; //X3_DESCENG
	{ ''																	, .T. }, ; //X3_PICTURE
...
aAdd( aSIX, { ;
	'ZAJ'																	, ; //INDICE
	'1'																		, ; //ORDEM
	'ZAJ_FILIAL+ZAJ_CLIENT+ZAJ_LOJA+ZAJ_DATA'								, ; //CHAVE
	'Cliente+Loja+Data'														, ; //DESCRICAO
	'Cliente+Loja+Data'														, ; //DESCSPA
	'Cliente+Loja+Data'														, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
	''																		, ; //NICKNAME
	'S'																		} ) //SHOWPESQ
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDVO" ) ) ) ;
