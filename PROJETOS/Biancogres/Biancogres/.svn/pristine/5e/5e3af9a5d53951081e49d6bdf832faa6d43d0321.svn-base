...
/*/
//--------------------------------------------------------------------
User Function UPDTAF02( cEmpAmb, cFilAmb ) 
...
Local   cDesc2    := "Autor: Facile - Data: 27/09/2018" 
Local   cDesc3    := ""
Local   cDesc4    := "Descrição: Campos que compoem o projeto de automacao financeira."
Local   cDesc5    := "Item.....: Clientes especiais."
Local   cDesc6    := ""
Local   cDesc7    := ""
Local   cMsg      := ""
Local   lOk       := .F.
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDTAF02" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDTAF02" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	{ 'ACY'																	, .T. }, ; //X3_ARQUIVO
	{ '12'																	, .T. }, ; //X3_ORDEM
	{ 'ACY_YSUMCE'															, .T. }, ; //X3_CAMPO
	{ 'N'																	, .T. }, ; //X3_TIPO
	{ 3																		, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'Qt.Dia.Esp'															, .T. }, ; //X3_TITULO
	{ 'Qt.Dia.Esp'															, .T. }, ; //X3_TITSPA
	{ 'Qt.Dia.Esp'															, .T. }, ; //X3_TITENG
	{ 'Qt.Dia.Esp'															, .T. }, ; //X3_DESCRIC
	{ 'Qt.Dia.Esp'															, .T. }, ; //X3_DESCSPA
	{ 'Qt.Dia.Esp'															, .T. }, ; //X3_DESCENG
	{ '@E 999 '																, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
aAdd( aSX3, { ;
	{ 'SA1'																	, .T. }, ; //X3_ARQUIVO
	{ 'Y7'																	, .T. }, ; //X3_ORDEM
	{ 'A1_YEMABOL'															, .T. }, ; //X3_CAMPO
	{ 'C'																	, .T. }, ; //X3_TIPO
	{ 150																	, .T. }, ; //X3_TAMANHO
	{ 0																		, .T. }, ; //X3_DECIMAL
	{ 'E-Mail Boleto'														, .T. }, ; //X3_TITULO
	{ 'E-Mail Boleto'														, .T. }, ; //X3_TITSPA
	{ 'E-Mail Boleto'														, .T. }, ; //X3_TITENG
	{ 'E-Mail Boleto'														, .T. }, ; //X3_DESCRIC
	{ 'E-Mail Boleto'														, .T. }, ; //X3_DESCSPA
	{ 'E-Mail Boleto'														, .T. }, ; //X3_DESCENG
	{ '@S40'																, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
	{ Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
	Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) + ;
...
aAdd( aSX7, { ;
	'ZK0_CODREG'															, ; //X7_CAMPO
	'001'																	, ; //X7_SEQUENC
	'Posicione("ZK1",1,xFilial("ZK1")+FwFldGet("ZK0_CODREG"),"ZK1_DESCRI")'	, ; //X7_REGRA
	'ZK0_DESCRE'															, ; //X7_CDOMIN
	'P'																		, ; //X7_TIPO
	''																		, ; //X7_SEEK
	''																		, ; //X7_ALIAS
	0																		, ; //X7_ORDEM
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDTAF02" ) ) ) ;
