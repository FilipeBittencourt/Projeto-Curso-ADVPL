...
//		If sVend >= mv_par07 .And. sVend <= mv_par08
			...
			IF ! EMPTY(CREPATU) .AND. SUBSTRING(CREPATU,1,1) = "1"
				...
				IF CEMPANT == "01"
					IF (SA1->A1_YVENDB2 = CREPATU .OR. SA1->A1_YVENDB3 = CREPATU) 
						If sVend >= mv_par07 .And. sVend <= mv_par08
							lVend := .T.
						ENDIF							
					END IF
				ELSE
					IF (SA1->A1_YVENDI2 = CREPATU .OR. SA1->A1_YVENDI3 = CREPATU) 
						If sVend >= mv_par07 .And. sVend <= mv_par08
							lVend := .T.
						ENDIF							
					END IF				
			...
			ELSE	
				If sVend >= cRepAtu .And. sVend <= cRepAtu
					lVend := .T.
				EndIf
			END IF
		cVend := Soma1(cVend, 1)
...
PutSx1("MR780A","17","Converte Moeda da Devolucao"," "," ","mv_chh","N",1,0,1,"C","","","","","mv_par17","Pela Devolucao ","Pela Devolucao ","Pela Devolucao ","","Pela Dt.NF Orig","Pela Dt.NF Orig","Pela Dt.NF Orig","","","","","","","","","",aHelpPor,aHelpEng,aHelpSpa)
