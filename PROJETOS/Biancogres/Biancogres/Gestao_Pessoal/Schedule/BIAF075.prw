...
/*/{Protheus.doc} BIAF075
@author Tiago Rossini Coradini
@since 02/05/2017
@version 1.0
@description Workflow de lista mensal de colaboradores
@obs OS: 3617-16 - Claudia Mara
@type function
/*/


User Function BIAF075()
	...
	For nCount := 1 To Len(aEmp)
		...
		cSQL := " SELECT RA_MAT, RA_CLVL, RA_NOME, RA_ADMISSA, RA_NUMCP, RA_SERCP, RJ_DESC, RA_YSEMAIL "
		cSQL += " FROM " + cSRA + " SRA "
		cSQL += " INNER JOIN " + cSRJ + " SRJ "
		cSQL += " ON RA_CODFUNC = RJ_FUNCAO "
		cSQL += " WHERE RA_FILIAL = " + ValToSQL(xFilial("SRA"))
		//cSQL += " AND RA_SITFOLH NOT IN ('A', 'D', 'F') "
		cSQL += " AND RA_SITFOLH NOT IN ('D') "
		cSQL += " AND RA_CATFUNC <> 'A' "
		//cSQL += " AND (RA_CLVL IN ('2120', '2220') OR RA_CLVL BETWEEN '3000' AND '3999') "
		cSQL += " AND SRA.D_E_L_E_T_ = '' "
		cSQL += " AND RJ_FILIAL = " + ValToSQL(xFilial("SRJ"))
		cSQL += " AND SRJ.D_E_L_E_T_ = '' "
		cSQL += " ORDER BY RA_YSEMAIL, RA_ADMISSA, RA_NOME "
...
Static Function fGetCab(cEmpName)
	...
	cRet += '<tr>
	cRet += '<div align="left">
	cRet += '<th width="1200" scope="col">'+ cEmpName +' - Lista mensal de colaboradores </th>
	cRet += '</div>
	cRet += '</tr>
	cRet += '</tbody>
	cRet += '</table>
	...
	cRet += '<th class = "cabtab" width="60" scope="col"> Matrícula </th>
	cRet += '<th class = "cabtab" width="60" scope="col"> Clase Valor </th>
	cRet += '<th class = "cabtab" width="200" scope="col"> Colaborador </th>
	cRet += '<th class = "cabtab" width="60" scope="col"> Dt. Admissão </th>
	cRet += '<th class = "cabtab" width="60" scope="col"> CTPS </th>	
	cRet += '<th class = "cabtab" width="200" scope="col"> Função </th>	
	cRet += '</tr>
	cRet += '</tbody>
	cRet += '</table>	
	...
	cRet += '<td class="style12" width="60"scope="col">'+ AllTrim((cQry)->RA_NUMCP) + "-" + AllTrim((cQry)->RA_SERCP) +'</td>
	...
	cRet += '<tbody>
	cRet += '<tr>
	cRet += '<th class="tottab" scope="col" width="300"> E-mail enviado automaticamente pelo sistema Protheus (by BIAF075).</th>
	cRet += '</tr>
	cRet += '</tbody>
	cRet += '</table>
	cRet += "</body>
	...
	Local cCopia := AllTrim(U_EmailWF('BIAF075', cEmp, cClvl))

	cMail += ";" + cCopia

	U_BIAEnvMail(,cMail, "Lista mensal de colaboradores", cHTML)
