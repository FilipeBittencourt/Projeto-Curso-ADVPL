...
/*/{Protheus.doc} BIAFM014
@author Marcelo 
@since 06/12/2018
@version 1.0
@description Relatório de acompanhamento do Banco de Horas
@type function
/*/

User Function BIAFM014()
	...
	If Select("SX6") == 0
		...
		For x := 1 to Len(xv_Emps)
			...
			Conout("BIAFM014 - Dia da semana corrente: " + Alltrim(DiaSemana( Date() )))
...
Static Function RptDetail(cReto)
	...
	ELSEIF cReto == 3
		...
		// Retirado em 24/06/19, pois identificamos que apresenta problema especificamente para o fechamento do Banco de Horas
		//MV_PAR01 := STOD(SUBSTR(GETMV("MV_PONMES"),1,8))
	...
	If !Empty(MV_PAR04)
		JF002 += "                       AND RTRIM(RA_YSEMAIL) = '"+Alltrim(MV_PAR04)+"' " + cEnter
	EndIf
	...
	JF002 += "  ORDER BY 2, 3, 6, 1 DESC " + cEnter
	...
	While !Eof()
		...
		kjSuper := Alltrim(JF02->SUPER)
		kjHrSpr := 0
		aColabHE := {}

		While !Eof() .and. Alltrim(JF02->SUPER) == kjSuper

			kjMatric := JF02->MATRIC
			kjHrMtr  := 0
			kjAcHrOr := 0

			While !Eof() .and. Alltrim(JF02->SUPER) == kjSuper .and. JF02->MATRIC == kjMatric
				...
				IF JF02->TPMOV < 0 .AND. JF02->XTATUS <> 'B' .AND. JF02->QUANT > 0

					cDeb := __TimeSum(cDeb,JF02->QUANT)

				ELSEIF JF02->TPMOV > 0 .AND. JF02->XTATUS <> 'B' .AND. JF02->QUANT > 0
		...
		IF (cReto <> 3) .AND. !EMPTY(cRet)
			...
			if !Empty(kjSuper)
				kjSuper += ";" + U_EmailWF('BIAFM014',cEmpAnt) 
			else
				kjSuper :=  U_EmailWF('BIAFM014',cEmpAnt) 
			endif 
...
Static Function fValidPerg()
	...
	aAdd(aRegs,{cPerg,"04","Filtrar por e-mail SUPER ?","","","mv_ch4","C",85,0,0,"G","","mv_par04","","","","","","","","","","","","","","","","","","","","","","","","",""})
	aAdd(aRegs,{cPerg,"05","Enviar Arq. p/ Supervisor?","","","mv_ch5","N",01,0,0,"C","","mv_par05","Sim","","","","","Não","","","","","","","","","","","","","","","","","","",""})
...
User Function BFM014Job()

	STARTJOB("U_BIAFM014()",GetEnvServer(),.F.,cEmpAnt,cFilAnt)
...
Static Function BH100(aColab, cSuperior)
	...
	For i := 1 to Len(aColab)
		...
		JF003 := "SELECT RA_MAT, RA_NOME, RA_YSEMAIL,SP9.P9_CODIGO+'-'+SP9.P9_DESC EVENTO, "+ cEnter
		...
		If JF03->(!Eof())
			...
			If !CriaArq
				...
				oExcel2:AddColumn(nxPlan2, nxTabl2, "e-mail superior"	,1,1)
