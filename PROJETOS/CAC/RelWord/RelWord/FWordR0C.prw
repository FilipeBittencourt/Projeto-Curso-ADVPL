...
Local cArqSaida := ""
Local oWord     := NIL
Local cArqWord  := "\Proposta_Pecas_atl.dot"
...
Local aUsu     := {}
	...
	If AB4->(DbSeek(xFilial("AB4")+AB3->AB3_NUMORC))  
        While !AB4->(Eof()) .And. xFilial("AB4")+AB4->AB4_NUMORC == xFilial("AB3")+AB3->AB3_NUMORC
			...
			aAdd(aExp, {"EQUIPAMENTO" + cValToChar(nEqp), ;
			            AllTrim(SB1->B1_DESC)+AllTrim(SB1->B1_YMODELO)+" série: "+ AllTrim(AB4->AB4_NUMSER) + ";", ;
			            "@!", "Equipamento"})
	...
	// =====================================================
	// AB3 Orçamento Tecnico, já utilizado no programa atual.
	// SQL SELECT  AB3_CODCLI, AB3_LOJA, AB3_EMISSA, * FROM AB3010 WHERE D_E_L_E_T_='' AND
    // AB3_NUMORC='003801' 
    // link entre as duas só XXX_FILIAL, XXX_CODCLI, XXX_LOJA, XXX_EMISSA  
    // AB1 Chamado Tenico, não esta sendo utilizado no programa, mas, nele tem alguns dos dados para o word
	...
	//Selcionando Apontamentos, SERVIÇOS E PEÇAS
	AB5->(DbSetOrder(1))
	...
	aAdd(aExp, {"N_PECAS", nPecas, "", "Qtd Peças"})		
