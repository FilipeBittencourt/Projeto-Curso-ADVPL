...
//			nValLiq	:= aProd[Nx][27] - aPisXml[1] - aCofinsXml[1] - aInssXml[1] - aIRRFXml[1] - aCSLLXml[1] - aIssRet[1]
		...
		EndIf
		if cCodMun == '3201308'
			cXml += '<unidmedida>'+Alltrim(aProd[Nx][08])+'</unidmedida>'
		endif 
		If cCodMun $ "3106200" .And. cTpPessoa == "EP" .And. cF4Agreg == "D"
			cString += '<aliquota>0.00</aliquota>'	  
		elseIf cCodMun $ "4115200"
			cString	+= "<aliquota>" + allTrim( iif( !empty( convType( aISSQN[1][2] ) ), AllTrim(Str(aISSQN[1][2])), AllTrim(Str(aISSRet[3])) ) ) + "</aliquota>"
		Else
