...
Method PLOGIN() CLASS TWSessionCodeAuth
  ...
  cPass	    := AllTrim("jascsp@321")                  // Parametro
    ...
    If !Empty(oReturn)
      If oReturn:ERROR:CODE == "E_VALIDATION_FAILED"
        For nW := 1 To Len(oReturn:ERROR:validations)
          cDetails += "<B>"+cValToChar(nW)+"</B> - "+ MsgParse(oReturn:ERROR:validations[nW]:field,  cLogin, cPass, cCNPJCPF)+ CRLF+ CRLF
        Next nW
        cDetails += cDetComp+ CRLF+ CRLF
        FwAlertWarning(AllTrim(cValToChar(cDetails)),"Atenção: "+cMsgTit+"")
        lRet := .F.
      ElseIf Empty(cExcept)
        cMsg := AllTrim(DecodeUTF8(oReturn:ERROR:message, "WINDOWS-1252"))+ CRLF+ CRLF
        cMsg += cDetComp
        FwAlertWarning( cMsg,"Error: "+cMsgTit+"")
        lRet := .F.
      EndIf
    Else
