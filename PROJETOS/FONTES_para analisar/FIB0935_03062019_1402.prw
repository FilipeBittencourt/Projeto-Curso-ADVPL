...
±±ºPrograma  ³FIB0822   ºAutor  ³Renato Coelho       º Data ³  18/06/15   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Relatório de Compromisso de Pagamento                      º±±
±±º          ³                                                            º±±
...
User Function FIB0935()
		...
		/*
		If !Pergunte("FIB0837")
			MsgStop("Operação cancelada.","Compromisso de Pagamento de Frete.")
			Return
		EndIf
		*/
		...
		//Montagem do Objeto para Impressão
		Private oPrint := FWMSPrinter():New("FIB0837",IMP_PDF,lAdjustToLegacy,,lDisableSetup,,,,,.F.)

		//Define a Resolução
		oPrint:SetResolution(72)

		//Define orientação da página para Paisagem, considerando (3508px X 2480px)
		oPrint:SetPortrait()
		...
		//Define o diretorio de gravação do arquivo. Caso não exiata o sistema define o temp do SO
	...
	cSQL += " SE2.E2_NUM 'E2NUM', "
	cSQL += " SE2.E2_VALOR 'E2VALOR' ,"
	cSQL += " SE2.E2_VENCTO  'E2VENCTO'  "
		...
		cSQL += "   AND SE2.E2_NUM = A.GWJ_NRPF
		...
		While (cWorkArea)->(!Eof())
	      ...
	      cSQL += " 	INNER JOIN " + RetSQLName("GW1") + " GW1 ON F1_CHVNFE = GW1.GW1_DANFE   AND GW1.GW1_FILIAL = '"+xFilial("GW1")+"' "
			...
			If Len(aAux)  <= 0
				Aviso("Atencao","Dados da nota fiscal chave ' " + cValToChar(Alltrim((cWorkArea)->(DANFE)))+ "' ,  não foram encontrados nos documentos(NF) de entrada/saida. ")  
					return .f.
			Endif

		 	If !Empty((cWorkArea)->(E2VALOR))
				cUser  := (cWorkArea)->(USUIMP)
				cUserDT  := (cWorkArea)->(DTLIB)
				cE2NUM  :=  (cWorkArea)->(E2NUM)
				nE2VALOR  :=  (cWorkArea)->(E2VALOR)
				dE2VENCTO  :=  (cWorkArea)->(E2VENCTO)
			Endif
		...
		For nX := 1 To Len(aInfoRel)
					...
					*/  
					...
					oPrint:Say (nLin, nLimEsq+20, "Número......: " + cNum + " ", oFont1,,,,) 
					//oPrint:Say (nLin, nLimEsq+235, cNum + IIf(cProvis = "S"," - [PROVISÃO]",""), oFont2,,,,) 
					oPrint:Say (nLin, nMrgMeio+20, "Vencimento........: ", oFont1,,,,)
					
					//oPrint:Say (nLin, nMrgMeio+330, DtoC(StoD(dVencto)), oFont2,,,,)
					oPrint:Say (nLin, nMrgMeio+330, DtoC(StoD(dE2VENCTO)), oFont2,,,,)
					
					nLin += 30
					
					oPrint:Say (nLin, nLimEsq+20, "Emissão.....: ", oFont1,,,,)
					...
					oPrint:Say (nLin, nMrgMeio+330, AllTrim(TransForm(nE2VALOR,"@E 999,999,999.99"))+" (Tit. "+AllTrim(cValToChar(cE2NUM))+")", oFont2,,,,)
					
					nLin += 30
					
					oPrint:Say (nLin, nLimEsq+20, "C. de Custo.: ----", oFont1,,,,)
					// oPrint:Say (nLin, nLimEsq+235, AllTrim(cCC), oFont2,,,,)   
					...
					oPrint:Say (nLin, nLimEsq+20, "Código Rota / Negociação.: " + AllTrim(cRota) +" ", oFont1,,,,)
					...
					oPrint:Say (nLin, nLimEsq+20, "Operação: " + cValToChar(IIF(cF1F2 == "F2", "VENDA","DEVOLUÇÃO/NEGOCIAÇÃO")) + " ", oFont1,,,,)
					...
					oPrint:Say (nLin, nLimEsq, "COMPOSIÇÃO DO(S) CONHECIMENTO(S) DE TRANSPORTE", oFont2,,,,)
					...
					oPrint:Say (nLin+30, nLimEsq+1537, "Pedágio", oFont2,,,,)
					oPrint:Say (nLin+30, nLimEsq+1687, "Descarga", oFont2,,,,)
					oPrint:Say (nLin+30, nLimEsq+1837, "Diária", oFont2,,,,)
					...
					//                                                                                                                                                            | 
					// Descricao : Impressão aglutinando dos CTR                                                                                                                  |
					//____________________________________________________________________________________________________________________________________________________________|
					...
					For nW := 1 To Len(aAux)
							...
							oPrint:Say (nLinAux+30, nLimEsq+345, SubStr(cNomeZ05,1,22), oFont1,,,,) //Redução para inclução de somatório e campo de processo
					...
					*/
					...
					//fechando rodapé de sessão das rotas
					nLinAux := nLin+30					
					oPrint:Line(nLin, nLimEsq, nLin+50, nLimEsq, , "-3")
					oPrint:Line(nLin, nLimDir, nLin+50, nLimDir, , "-3")				
					nLin += 50					
					...
					*/
					//Observações
					oPrint:Say (nLin, nLimEsq, "OBSERVAÇÕES", oFont2,,,,)
					...
					oPrint:Say (nLin, nLimEsq, "LANÇAMENTO E AUTORIZAÇÃO", oFont2,,,,)
					...
					nLin += 30
					oPrint:SayAlign(nLin, nLimEsq, "Data de Lançamento", oFont1, nTamSec, 30, CLR_BLACK, 2, 0)
					//oPrint:SayAlign(nLin, nLimEsq, , oFont1, nTamSec, 30, CLR_BLACK, 2, 0)
					
					oPrint:SayAlign(nLin, nLimEsq+nTamSec, "Data de Emissão", oFont1, nTamSec, 30, CLR_BLACK, 2, 0)
					oPrint:SayAlign(nLin, nLimEsq+(nTamSec*2), "Data da Aprovação", oFont1, nTamSec, 30, CLR_BLACK, 2, 0)
					...
					oPrint:SayAlign(nLin, nLimEsq+nTamSec, "Supervisão", oFont1, nTamSec, 30, CLR_BLACK, 2, 0)
					oPrint:SayAlign(nLin, nLimEsq+(nTamSec*2), "Gerência/Diretoria", oFont1, nTamSec, 30, CLR_BLACK, 2, 0)
...
//Verifica a existencia das Perguntas (SX1) se não existir inclui.
...
aAdd(aRegs,{cPerg,"03","Dta Emissão de    :","","","mv_ch3" ,"D",08,0,0,"G","","mv_par03","","","","","","","","","","","","","","","","","","","","","","","","",""})
aAdd(aRegs,{cPerg,"04","Dta Emissão até   :","","","mv_ch4" ,"D",08,0,0,"G","","mv_par04","","","","","","","","","","","","","","","","","","","","","","","","",""})
